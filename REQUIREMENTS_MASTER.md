...
## 16. Access management & list/action usability standard — V30.40+ PERMANENT RULE
- Normal access administration is assignment-first: User → one or more Access Profiles → BU scope per profile → optional Permission Groups → separate Approval Authority → Effective Access → Review & Confirm. Granular user overrides belong in Advanced Access and should be rare.
- Multiple Access Profiles per user are supported and additive. Profile scope is independent per assignment; a user may hold different responsibilities in different BUs. A Primary Profile is descriptive/default context only and must not cancel other profiles.
- Shared Access Profiles and Permission Groups are reusable job/functional bundles. Profile library mutation is tightly controlled; delegated administrators may not grant authority, limits or BU scope above their own. Every access mutation is audited.
- Approval Authority/L0–L5 is separate from ordinary feature permissions. L5 remains the protected dual Finance + CEO path. Multiple profiles must never bypass maker/checker or segregation-of-duties controls.
- Existing users must migrate without unexpected gain/loss of effective access. Access model upgrades are additive and must preserve audit/history.
- Every list with an Actions column places Actions at the far right and right-aligns action controls. On genuinely wide tables, horizontal scrolling is allowed and the Actions column should remain sticky/pinned where practical. Do not globally ban horizontal scrolling or hide important business data just to make a table fit.
- Short/medium lists should fit available width using sensible sizing/wrapping. Long narrative cells such as Accounting Proposal may wrap to multiple lines.
- Do not show redundant navigation to the page/section already active. Preserve meaningful Back/context actions.
- Filter panels default collapsed where used, with compact clear UI; avoid redundant headings such as an extra Date Range label when From/To fields are already self-explanatory.
- Successful child-modal mutations auto-close the child and refresh only the affected parent scope. Receiver/payee account-number validation, Cash-vs-electronic account requirements, browser new-tab navigation and Chrome language metadata are system-wide consistency rules.
- Verified financial/operational records do not expose normal Void actions to ordinary users. CEO / Owner controlled reversal/void authority remains audited and subject to existing integrity rules.

## 17. Access Control Center & simple user assignment — V30.41+ PERMANENT RULE
- Separate **access-structure configuration** from **day-to-day user assignment**. System Settings → Security & Access is the authoritative place to design and maintain Access Profiles, Permission Groups, Access Policies, Approval Levels and Access Audit. Users & Access is primarily for creating users and assigning approved structures.
- **Access Profiles** are reusable job/responsibility bundles and may contain multiple module/action permissions, sensitive permissions, delegated permissions and configured limits. A user may receive multiple active Access Profiles, and each profile assignment may have its own Business Unit scope. One profile may be marked Primary only for legacy/display/default context; all active profile grants remain additive.
- **Permission Groups** are reusable add-on bundles and may contain multiple related permissions in one group. A user may receive multiple Permission Groups, each optionally scoped to a Business Unit. Groups complement profiles and should prevent administrators from creating many nearly-identical profiles for small responsibility differences.
- **Access Policies** are reusable restriction/policy sets and may contain multiple rules/conditions in one policy. Supported policy concepts include deny action, minimum Approval Level, required permission, required Permission Group and CEO/Owner-only action. Policies may be company-wide or BU-scoped. Policies restrict runtime access and never create/grant missing feature permission.
- **Approval Authority** remains separate from ordinary permissions. L0–L4 may define configurable default approval/payment/expense limits; L5 remains protected dual approval and is not an ordinary single-user authority level.
- User creation/edit keeps **Job Title / Position** separate from system access. Creation must support selecting Business Units, one or more Access Profiles, BU scope per profile, optional multiple Permission Groups, separate Approval Authority/limits and an Effective Access preview before Review & Confirm.
- Normal access administration must remain simple: User → Profile(s) → BU Scope → optional Group(s) → Approval Authority → Effective Access → Review & Confirm. Granular user-level Allow/Deny exceptions and individual limits remain under Advanced Access for exceptional cases.
- Effective Access must expose the source of grants (Profile / Permission Group / individual exception) and remain authoritative at the backend. UI visibility alone never constitutes permission enforcement.
- Shared Access Profile / Permission Group / Access Policy changes must show affected-user impact where practical before confirmation. Archive must be protected when an assigned reusable structure cannot safely be removed. All structural and assignment changes are audited.
- CEO/Owner and authorized System Administrators may manage shared access structures. Delegated administrators may assign approved structures only within their BU/authority and may never grant permissions, limits, scopes or approval authority above their own effective authority.
