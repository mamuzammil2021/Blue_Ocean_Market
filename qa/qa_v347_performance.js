...
async function main(){
 ...
 ok(pkg.version==='30.51.0'&&lock.version==='30.51.0'&&lock.packages[''].version==='30.51.0','release package and lock identities');
 ok(server.includes("version:'30.51.0'")&&server.includes('Blue Ocean Market V30.51.0 running'),'server and health identity');
 ok(html.includes('/v347-loader.js?v=30.51.0')&&!html.includes('<script src="/v347-client.js'),'workspaces are actually lazy, not eagerly duplicated');
 ...
 console.log(`\nV30.51.0 PERFORMANCE QA PASS (${passed} checks)`);
}
