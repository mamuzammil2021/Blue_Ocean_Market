...
const pkg=JSON.parse(read('package.json')),html=read('public/index.html'),runtime=read('public/runtime-v3039.js'),server=read('server/server.js'),db=read('server/db.js'),req=read('REQUIREMENTS_MASTER.md');
check('package version 30.39.0',pkg.version==='30.39.0');
...
check('runtime bundle loaded',/runtime-v3039\.js\?v=30\.39\.0/.test(html));
...
for(const f of ['public/i18n-ko.js','public/client.js','public/runtime-v3039.js']){const gz=f+'.gz';check(`${gz} exists`,fs.existsSync(path.join(root,gz)));if(fs.existsSync(path.join(root,gz)))check(`${gz} smaller than source`,fs.statSync(path.join(root,gz)).size<fs.statSync(path.join(root,f)).size)}
...
check('server reports V30.39.0',server.includes('Blue Ocean Market V30.39.0 running on port'));
...
check('document observer baseline does not grow above V30.39 audit threshold',observers<=31,`found ${observers}`);
...
for(const f of ['server/server.js','server/v339-performance.js','server/pagination-v339.js','server/async-pdf-v339.js','server/v307.js','server/v308.js','server/v313.js','server/v315.js','public/v339-client.js','public/runtime-v3039.js']){try{child.execFileSync(process.execPath,['--check',path.join(root,f)],{stdio:'pipe'});check(`syntax ${f}`,true)}catch(e){check(`syntax ${f}`,false,String(e.stderr||e.message))}}
