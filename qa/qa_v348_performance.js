...
(async function(){
 ...
 ok(p.version==='30.51.0'&&lock.version==='30.51.0'&&lock.packages[''].version==='30.51.0','current package/lock identity');
 ok(server.includes("version:'30.51.0'")&&server.includes('Blue Ocean Market V30.51.0 running'),'server health/runtime identity');
 ok(idx.includes('v348-bell.js?v=30.51.0')&&idx.includes('v348-loader.js?v=30.51.0')&&!idx.includes('<script src="/v348-notifications.js'),'small bell/loader boot; scroll view remains lazy');
 ...
 ok(bell.includes('/api/v348/notifications/feed?pageSize=25&status=all')&&!bell.includes("api('/api/notifications')")&&bell.includes('d.unread_total'),'bell preview no longer downloads entire history; accurate unread count');
 ...
 console.log(`\nV30.51.0 SMART PERFORMANCE QA PASS (${passed} checks)`);
