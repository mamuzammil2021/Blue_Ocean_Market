...
function install({app,db,auth,allow,currentUnit,enforceUnit,audit,notify,uploads,financeSync,voidFinanceBySource,accounting,configuredPaymentAccount,upload:sharedUpload}){
  ...
  function flushAccounting(){try{accounting?.scheduleQueue?.(0)}catch(e){console.error('Pink Salt accounting queue:',e.message)}}
  ...
  function backfillPinkSaltFinance(){try{
    ...
    const migrationKey='v30.39.2-pink-salt-finance-backfill';
    if(db.prepare('SELECT 1 FROM system_migrations WHERE migration_key=?').get(migrationKey))return {skipped:true};
    ...
    flushAccounting();
    db.prepare('INSERT OR REPLACE INTO system_migrations(migration_key,applied_at,details) VALUES(?,CURRENT_TIMESTAMP,?)').run(migrationKey,'Historic Pink Salt Finance/Accounting backfill completed once under V30.39.2.');
    return {skipped:false};
  }catch(e){console.error('V30.1 Pink Salt Finance backfill:',e.message);return {error:String(e.message||e)}}}
  ...
  app.post('/api/pink-salt/imports',auth,allow('purchases','inventory','finance'),upload.array('attachments',12),(req,res)=>{
    ...
    const tx=db.transaction(()=>{
      ...
      const no=text(req.body.import_no)||numbering.next('pink_salt.import',{businessUnitId:bu});
  ...
  app.post('/api/pink-salt/imports/:id/payments',auth,allow('purchases','finance'),upload.single('receipt'),(req,res)=>{const bu=guard(req,res);if(!bu)return;const imp=db.prepare('SELECT * FROM pink_salt_imports WHERE id=? AND business_unit_id=?').get(req.params.id,bu);if(!imp)return res.status(404).json({error:'Import not found.'});if(imp.status==='Received')return res.status(409).json({error:'Supplier payments cannot be added after the shipment has been received. Use an approved Finance correction if required.'});if(['Cancelled','Voided'].includes(imp.status))return res.status(409).json({error:'Supplier payments cannot be added to a cancelled or voided import.'});const currency=text(req.body.original_currency)||imp.invoice_currency||'KRW',amount=num(req.body.original_amount),rate=currency==='KRW'?1:num(req.body.fx_rate_to_krw),method=text(req.body.payment_method)||'Bank',reference=text(req.body.reference);if(amount<=0||rate<=0||(method.toLowerCase()!=='cash'&&!reference))return res.status(400).json({error:method.toLowerCase()==='cash'?'Payment amount and exchange rate are required.':'Payment amount, exchange rate and reference are required for non-cash payments.'});if(!req.file)return res.status(400).json({error:'Payment receipt / evidence is required.'});let paymentAccount=null;try{paymentAccount=configuredPaymentAccount(req,bu,method,req.body.payment_account_id,{requiredForNonCash:true})}catch(e){return res.status(e.status||400).json({error:e.message})}const krw=amount*rate,before=importFinancialState(imp.id);if(krw>(before?.outstanding_krw||0)+0.01)return res.status(409).json({error:`Supplier payment exceeds the outstanding import balance of ₩${Math.round(before?.outstanding_krw||0).toLocaleString()}.`,outstanding_krw:before?.outstanding_krw||0});const paymentDate=req.body.payment_date||today(),dup=reference?db.prepare("SELECT id FROM pink_salt_import_payments WHERE import_id=? AND payment_date=? AND lower(trim(reference))=lower(trim(?)) AND ABS(krw_amount-?)<0.01 AND COALESCE(status,'Active')!='Voided' LIMIT 1").get(imp.id,paymentDate,reference,krw):null;if(dup)return res.status(409).json({error:`This supplier payment is already recorded as payment #${dup.id}.`,existing_id:dup.id});const r=db.prepare('INSERT INTO pink_salt_import_payments(import_id,payment_date,original_currency,original_amount,fx_rate_to_krw,krw_amount,payment_method,reference,receipt_file,notes,created_by,payment_account_id) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)').run(imp.id,paymentDate,currency,amount,rate,krw,method,reference,req.file.filename,text(req.body.notes),req.user.id,paymentAccount?.id||null),id=Number(r.lastInsertRowid);saveAttachment(bu,req.user,'Import',imp.id,req.file,'Import Payment Receipt / Evidence');audit(req.user,'pink_salt_import_payment',id,'create',JSON.stringify({import_id:imp.id,krw}));syncImportPayment(id);const after=importFinancialState(imp.id);if(after.ready_to_receive)notifyPink(bu,'info','Pink Salt import fully paid',`${imp.import_no}: supplier payment is fully cleared. Shipment is ready to receive.`,'pink_salt_import',imp.id,'psImports',{finance:true,managers:true,staff:true});else notifyPink(bu,'warning','Pink Salt supplier payment recorded',`${imp.import_no}: ₩${Math.round(after.outstanding_krw).toLocaleString()} remains outstanding before shipment can be received.`,'pink_salt_import',imp.id,'psImports',{finance:true,managers:true});res.json({id,krw_amount:krw,...after})});

  app.post('/api/pink-salt/imports/:id/costs',auth,allow('purchases','finance'),upload.single('receipt'),(req,res)=>{const bu=guard(req,res);if(!bu)return;const imp=db.prepare('SELECT * FROM pink_salt_imports WHERE id=? AND business_unit_id=?').get(req.params.id,bu);if(!imp)return res.status(404).json({error:'Import not found.'});if(['Cancelled','Voided'].includes(imp.status))return res.status(409).json({error:'Import costs cannot be added to a cancelled or voided import.'});const currency=text(req.body.original_currency)||'KRW',amount=num(req.body.original_amount),rate=currency==='KRW'?1:num(req.body.fx_rate_to_krw),reference=text(req.body.reference),method=text(req.body.payment_method)||'Bank';if(amount<=0||rate<=0||!text(req.body.cost_type)||(method.toLowerCase()!=='cash'&&!reference))return res.status(400).json({error:method.toLowerCase()==='cash'?'Cost type, amount and exchange rate are required.':'Cost type, amount, exchange rate and reference are required for non-cash payments.'});if(!req.file)return res.status(400).json({error:'Import-cost receipt / evidence is required.'});let paymentAccount=null;try{paymentAccount=configuredPaymentAccount(req,bu,method,req.body.payment_account_id,{requiredForNonCash:true})}catch(e){return res.status(e.status||400).json({error:e.message})}const krw=amount*rate,costDate=req.body.cost_date||today(),costType=text(req.body.cost_type),dup=reference?db.prepare("SELECT id FROM pink_salt_import_costs WHERE import_id=? AND cost_date=? AND cost_type=? AND lower(trim(reference))=lower(trim(?)) AND ABS(krw_amount-?)<0.01 AND COALESCE(status,'Active')!='Voided' LIMIT 1").get(imp.id,costDate,costType,reference,krw):null;if(dup)return res.status(409).json({error:`This import cost is already recorded as cost #${dup.id}.`,existing_id:dup.id});const r=db.prepare('INSERT INTO pink_salt_import_costs(import_id,cost_date,cost_type,original_currency,original_amount,fx_rate_to_krw,krw_amount,allocation_method,reference,receipt_file,notes,created_by,payment_method,payment_account_id) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)').run(imp.id,costDate,costType,currency,amount,rate,krw,text(req.body.allocation_method)||'Weight',reference,req.file.filename,text(req.body.notes),req.user.id,method,paymentAccount?.id||null),id=Number(r.lastInsertRowid);saveAttachment(bu,req.user,'Import',imp.id,req.file,`${text(req.body.cost_type)} Evidence`);importLandedRecalc(imp.id);audit(req.user,'pink_salt_import_cost',id,'create',JSON.stringify({import_id:imp.id,krw}));syncImportCost(id);res.json({id,krw_amount:krw})});
  ...
  // V30.39.2: customer account summaries are calculated in bulk rather than
  // orderFinancials() once per order per customer (classic N+1 query amplification).
  app.get('/api/pink-salt/customers',auth,allow('sales','finance','dashboard'),(req,res)=>{const bu=guard(req,res);if(!bu)return;const rows=db.prepare(`
    WITH pay AS (SELECT order_id,SUM(krw_amount) paid FROM pink_salt_customer_payments WHERE status='Active' GROUP BY order_id),
    ref AS (SELECT order_id,SUM(krw_amount) refunded FROM pink_salt_customer_refunds WHERE status='Active' GROUP BY order_id),
    ofn AS (SELECT o.id,o.customer_id,o.status,o.total_krw,CASE WHEN COALESCE(pay.paid,0)-COALESCE(ref.refunded,0)>0 THEN COALESCE(pay.paid,0)-COALESCE(ref.refunded,0) ELSE 0 END net_paid FROM pink_salt_orders o LEFT JOIN pay ON pay.order_id=o.id LEFT JOIN ref ON ref.order_id=o.id WHERE o.business_unit_id=? AND o.status IN ('Completed','Cancelled')),
    agg AS (SELECT customer_id,COUNT(CASE WHEN status='Completed' THEN 1 END) completed_orders,SUM(CASE WHEN status='Completed' THEN total_krw ELSE 0 END) lifetime_sales_krw,SUM(net_paid) paid_krw,SUM(CASE WHEN status='Cancelled' THEN 0 WHEN total_krw-net_paid>0 THEN total_krw-net_paid ELSE 0 END) outstanding_krw,SUM(CASE WHEN status='Cancelled' THEN net_paid WHEN net_paid-total_krw>0 THEN net_paid-total_krw ELSE 0 END) customer_credit_krw FROM ofn GROUP BY customer_id)
    SELECT c.*,COALESCE(a.completed_orders,0) completed_orders,COALESCE(a.lifetime_sales_krw,0) lifetime_sales_krw,COALESCE(a.paid_krw,0) paid_krw,COALESCE(a.outstanding_krw,0) outstanding_krw,COALESCE(a.customer_credit_krw,0) customer_credit_krw FROM pink_salt_customers c LEFT JOIN agg a ON a.customer_id=c.id WHERE c.business_unit_id=? ORDER BY c.active DESC,lower(c.name),c.id`).all(bu,bu);res.json(rows)});
  ...
  // V30.39.2: order cash state is aggregated once for the complete list.
  app.get('/api/pink-salt/orders',auth,allow('sales','finance','dashboard','inventory'),(req,res)=>{const bu=guard(req,res);if(!bu)return;const rows=db.prepare(`
    WITH pay AS (SELECT order_id,SUM(krw_amount) paid FROM pink_salt_customer_payments WHERE status='Active' GROUP BY order_id),
    ref AS (SELECT order_id,SUM(krw_amount) refunded FROM pink_salt_customer_refunds WHERE status='Active' GROUP BY order_id),
    items AS (SELECT order_id,COUNT(*) item_count FROM pink_salt_order_items GROUP BY order_id)
    SELECT o.*,c.name customer,COALESCE(items.item_count,0) item_count,COALESCE(pay.paid,0) paid_gross_krw,COALESCE(ref.refunded,0) refunded_krw FROM pink_salt_orders o LEFT JOIN pink_salt_customers c ON c.id=o.customer_id LEFT JOIN pay ON pay.order_id=o.id LEFT JOIN ref ON ref.order_id=o.id LEFT JOIN items ON items.order_id=o.id WHERE o.business_unit_id=? ORDER BY o.order_date DESC,o.id DESC`).all(bu).map(o=>{const net=Math.max(0,num(o.paid_gross_krw)-num(o.refunded_krw));return {...o,paid_krw:net,outstanding_krw:o.status==='Cancelled'?0:Math.max(0,num(o.total_krw)-net),customer_credit_krw:o.status==='Cancelled'?Math.max(0,net):Math.max(0,net-num(o.total_krw))}});res.json(rows)});
  ...
  app.post('/api/pink-salt/orders/:id/payments',auth,allow('sales','finance'),upload.single('receipt'),(req,res)=>{const bu=guard(req,res);if(!bu)return;const o=db.prepare('SELECT * FROM pink_salt_orders WHERE id=? AND business_unit_id=?').get(req.params.id,bu);if(!o)return res.status(404).json({error:'Order not found.'});if(o.status==='Cancelled')return res.status(409).json({error:'Use Customer Refund for a cancelled order.'});const currency=text(req.body.original_currency)||'KRW',original=num(req.body.original_amount),rate=currency==='KRW'?1:num(req.body.fx_rate_to_krw),amount=original*rate,reference=text(req.body.reference),method=text(req.body.payment_method)||'Bank';if(original<=0||rate<=0||(method.toLowerCase()!=='cash'&&!reference))return res.status(400).json({error:method.toLowerCase()==='cash'?'Payment amount and exchange rate are required.':'Payment amount, exchange rate and reference are required for non-cash payments.'});if(!req.file)return res.status(400).json({error:'Customer payment receipt / evidence is required.'});let paymentAccount=null;try{paymentAccount=configuredPaymentAccount(req,bu,method,req.body.payment_account_id,{requiredForNonCash:true})}catch(e){return res.status(e.status||400).json({error:e.message})}const current=orderFinancials(o.id),remaining=Math.max(0,num(o.total_krw)-num(current?.net_paid_krw));if(amount>remaining+0.01)return res.status(409).json({error:`Payment exceeds this order's outstanding amount of ₩${Math.round(remaining).toLocaleString()}.`});const paymentDate=req.body.payment_date||today(),dup=reference?db.prepare("SELECT id FROM pink_salt_customer_payments WHERE order_id=? AND payment_date=? AND lower(trim(reference))=lower(trim(?)) AND ABS(krw_amount-?)<0.01 AND COALESCE(status,'Active')!='Voided' LIMIT 1").get(o.id,paymentDate,reference,amount):null;if(dup)return res.status(409).json({error:`This customer payment is already recorded as payment #${dup.id}.`,existing_id:dup.id});const role=o.status==='Reserved'?'Advance':'Sale Payment',r=db.prepare("INSERT INTO pink_salt_customer_payments(business_unit_id,order_id,customer_id,payment_date,payment_role,original_currency,original_amount,fx_rate_to_krw,krw_amount,payment_method,reference,receipt_file,status,notes,created_by,payment_account_id) VALUES(?,?,?,?,?,?,?,?,?,?,?,?, 'Active',?,?,?)").run(bu,o.id,o.customer_id||null,paymentDate,role,currency,original,rate,amount,method,reference,req.file.filename,text(req.body.notes),req.user.id,paymentAccount?.id||null),id=Number(r.lastInsertRowid);saveAttachment(bu,req.user,'Order',o.id,req.file,`${role} Receipt / Evidence`);audit(req.user,'pink_salt_customer_payment',id,'create',JSON.stringify({order_id:o.id,krw_amount:amount,payment_role:role}));syncCustomerPayment(id);const payment_status=refreshOrderPaymentStatus(o.id),f=orderFinancials(o.id);notifyPink(bu,f.outstanding_krw<=0.01?'info':'warning',f.outstanding_krw<=0.01?'Pink Salt customer payment cleared':'Pink Salt customer payment received',`${o.order_no}: ₩${Math.round(amount).toLocaleString()} received. Outstanding ₩${Math.round(f.outstanding_krw).toLocaleString()}.`,'pink_salt_order',o.id,'psSales',{finance:true,managers:true});res.json({id,krw_amount:amount,payment_role:role,payment_status,paid_krw:f.net_paid_krw,outstanding_krw:f.outstanding_krw})});

  app.post('/api/pink-salt/orders/:id/refunds',auth,allow('sales','finance'),upload.single('receipt'),(req,res)=>{const bu=guard(req,res);if(!bu)return;const o=db.prepare('SELECT * FROM pink_salt_orders WHERE id=? AND business_unit_id=?').get(req.params.id,bu);if(!o)return res.status(404).json({error:'Order not found.'});if(o.status!=='Cancelled')return res.status(409).json({error:'Customer refunds are available after the order is cancelled. Cancel the order first so stock and accounting are reversed correctly.'});const currency=text(req.body.original_currency)||'KRW',original=num(req.body.original_amount),rate=currency==='KRW'?1:num(req.body.fx_rate_to_krw),amount=original*rate,reference=text(req.body.reference),reason=text(req.body.reason),method=text(req.body.payment_method)||'Bank';if(original<=0||rate<=0||(method.toLowerCase()!=='cash'&&!reference)||!reason)return res.status(400).json({error:method.toLowerCase()==='cash'?'Refund amount, exchange rate and reason are required.':'Refund amount, exchange rate, reference and reason are required for non-cash payments.'});if(!req.file)return res.status(400).json({error:'Customer refund receipt / evidence is required.'});let paymentAccount=null;try{paymentAccount=configuredPaymentAccount(req,bu,method,req.body.payment_account_id,{requiredForNonCash:true});if(paymentAccount&&String(paymentAccount.currency||'KRW').trim().toUpperCase()!==String(currency).trim().toUpperCase())throw new Error(`Pay From account currency must match Refund Currency (${String(currency).toUpperCase()}). Use an explicit FX conversion workflow for cross-currency funding.`)}catch(e){return res.status(e.status||400).json({error:e.message})}const f=orderFinancials(o.id),available=num(f?.customer_credit_krw);if(amount>available+0.01)return res.status(409).json({error:`Refund exceeds available customer credit of ₩${Math.round(available).toLocaleString()}.`});const refundDate=req.body.refund_date||today(),dup=reference?db.prepare("SELECT id FROM pink_salt_customer_refunds WHERE order_id=? AND refund_date=? AND lower(trim(reference))=lower(trim(?)) AND ABS(krw_amount-?)<0.01 AND COALESCE(status,'Active')!='Voided' LIMIT 1").get(o.id,refundDate,reference,amount):null;if(dup)return res.status(409).json({error:`This customer refund is already recorded as refund #${dup.id}.`,existing_id:dup.id});const r=db.prepare("INSERT INTO pink_salt_customer_refunds(business_unit_id,order_id,customer_id,refund_date,original_currency,original_amount,fx_rate_to_krw,krw_amount,payment_method,reference,receipt_file,reason,status,notes,created_by,payment_account_id) VALUES(?,?,?,?,?,?,?,?,?,?,?,?, 'Active',?,?,?)").run(bu,o.id,o.customer_id||null,refundDate,currency,original,rate,amount,method,reference,req.file.filename,reason,text(req.body.notes),req.user.id,paymentAccount?.id||null),id=Number(r.lastInsertRowid);saveAttachment(bu,req.user,'Order',o.id,req.file,'Customer Refund Receipt / Evidence');audit(req.user,'pink_salt_customer_refund',id,'create',JSON.stringify({order_id:o.id,krw_amount:amount}));syncCustomerRefund(id);const payment_status=refreshOrderPaymentStatus(o.id),after=orderFinancials(o.id);notifyPink(bu,'info','Pink Salt customer refund recorded',`${o.order_no}: ₩${Math.round(amount).toLocaleString()} refunded. Remaining customer credit ₩${Math.round(after.customer_credit_krw).toLocaleString()}.`,'pink_salt_order',o.id,'psSales',{finance:true,managers:true});res.json({id,krw_amount:amount,payment_status,customer_credit_krw:after.customer_credit_krw})});
