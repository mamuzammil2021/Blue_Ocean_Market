...
function install(ctx={}){
  ...
  app.put('/api/finance/:id',auth,upload.array('attachments',20),(req,res)=>{
    try{
      ...
      if(c.task_id){try{db.prepare("UPDATE tasks SET status='Waiting',progress_percent=90,review_status='Pending',system_managed=1,workflow_state='Awaiting Finance Verification',action_enabled=0,updated_at=CURRENT_TIMESTAMP WHERE id=?").run(c.task_id);db.prepare('INSERT INTO task_history(task_id,user_id,action,note) VALUES(?,?,?,?)').run(c.task_id,req.user.id,'Corrected & Resubmitted',response)}catch(_){ }}
