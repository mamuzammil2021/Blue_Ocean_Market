...
function install(ctx){
  ...
  function paymentRefundedPkr(paymentId){const legacy=num(db.prepare("SELECT COALESCE(SUM(amount_pkr),0) v FROM excavator_resale_profit_refunds WHERE payment_id=? AND status='Completed'").get(Number(paymentId))?.v);let pooled=0;try{pooled=num(db.prepare("SELECT COALESCE(SUM(amount_pkr),0) v FROM excavator_resale_credit_refund_sources s JOIN excavator_resale_credit_refunds r ON r.id=s.refund_id WHERE s.payment_id=? AND r.status='Completed'").get(Number(paymentId))?.v)}catch(_){ }return round2(legacy+pooled)}
  ...
  function packageBuyerResales(buyer,bu){
    ...
    const payments=db.prepare(`SELECT p.*,a.name payment_account_name,a.account_country,a.currency account_currency,
      ...
      COALESCE((SELECT SUM(x.amount_pkr) FROM excavator_resale_profit_refunds x WHERE x.payment_id=p.id AND x.status='Completed'),0)+COALESCE((SELECT SUM(s.amount_pkr) FROM excavator_resale_credit_refund_sources s JOIN excavator_resale_credit_refunds rr ON rr.id=s.refund_id WHERE s.payment_id=p.id AND rr.status='Completed'),0) refunded_pkr
  ...
  app.post('/api/v327/excavator/buyers/:id/resale-payments/:pid/refund',auth,allow('finance'),upload.single('receipt'),(req,res)=>{
    ...
    try{
      ...
      const out=db.transaction(()=>{
        ...
        const financeId=financeSync({businessUnitId:g.bu,type:'Expense',category:'Pakistan Resale Profit Refund',amount:krw,description:`Refund Pakistan resale unallocated credit · ${g.buyer.name}`,paymentMethod:'Bank',reference,paymentAccountId:account.id,createdBy:req.user.id,sourceType:'Pakistan Resale Profit Refund',sourceId:rid,receiptFile:req.file.filename,originalAmount:amount,originalCurrency:'PKR',fxRate:fx,transactionDate:date,sourceLabel:'Excavator → Pakistan Resale Profit Refund',sourceRecordId:g.buyer.id,sourcePaymentId:pmt.id,receiverAccountId:Number(req.body.receiver_account_id||0),receiverEntityType:'excavator_buyer',receiverEntityId:g.buyer.id,receiverName:g.buyer.name});
