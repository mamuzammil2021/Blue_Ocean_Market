...
function install({app,db,auth,allow,currentUnit,enforceUnit,isFinanceReviewer}){
  ...
  function statementPdf(statement,lang='ko'){
    ...
    const koReplacements={
      'Machine Purchase':'장비 구매','Payment to Supplier':'공급업체 지급','Buyer Payment':'구매자 결제','Advance Allocation to Machine':'장비 선급금 배정','Legacy Direct Machine Allocation':'기존 장비 직접 배정','Buyer Advance Refund':'구매자 선급금 환불','Resale Share':'재판매 지분','Excavator Purchase Token':'굴삭기 구매 계약금','Excavator Purchase Payment':'굴삭기 구매 대금','Excavator Buyer Payment':'굴삭기 구매자 결제','Transport / Logistics':'운송 / 물류','Repair / Maintenance':'수리 / 유지보수','Parts / Spare Parts':'부품 / 예비 부품','Pakistan Resale Profit Payment':'파키스탄 재판매 이익 결제','Pakistan Resale Profit Refund':'파키스탄 재판매 이익 환불'
    };
    function localizeData(v){let out=String(v??'');if(lang!=='ko')return out;for(const [a,b] of Object.entries(koReplacements))out=out.replace(new RegExp(a.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),'gi'),b);return out}
    function cleanLatinPunctuation(v){let out=String(v??'');if(!/[가-힣ㄱ-ㅎㅏ-ㅣ]/.test(out))out=out.replace(/\s*·\s*/g,' - ').replace(/\s*→\s*/g,' -> ');return out}
    function display(v,translateLabel=false){let out=translateLabel?tr(String(v??''),lang):localizeData(v);return cleanLatinPunctuation(out)}
    function push(v){ops.push(v)}
    function tokenValue(value,translateLabel=false){const v=display(value,translateLabel);return /[^\x20-\x7e]/.test(v)?unicodeHex(v):`(${escPdf(v)})`}
    function text(x,yy,value,size=8,bold=false,translateLabel=false){const shown=display(value,translateLabel),unicode=/[^\x20-\x7e]/.test(shown),font=unicode?(bold?'F4':'F3'):(bold?'F2':'F1');push(`BT /${font} ${size} Tf 0.10 0.14 0.22 rg 1 0 0 1 ${Number(x).toFixed(1)} ${Number(yy).toFixed(1)} Tm ${tokenValue(value,translateLabel)} Tj ET`)}
    function charWidth(ch,size){if(/[가-힣ㄱ-ㅎㅏ-ㅣ一-龥]/.test(ch))return size*.95;if(/[MW@#%&]/.test(ch))return size*.72;if(/[ilI1.,:;|'`]/.test(ch))return size*.28;return size*.50}
    function approxWidth(value,size=8,translateLabel=false){return [...display(value,translateLabel)].reduce((n,ch)=>n+charWidth(ch,size),0)}
    function rightText(right,yy,value,size=8,bold=false,translateLabel=false){text(Math.max(M,right-approxWidth(value,size,translateLabel)),yy,value,size,bold,translateLabel)}
    ...
    function wrapWidth(value,maxWidth,size=7,maxLines=6,{translateLabel=false,semantic=false}={}){
      let raw=display(value,translateLabel).replace(/\r/g,'').trim();if(!raw)return [];
      const source=[];
      for(const part of raw.split(/\n+/)){const pieces=semantic?part.split(/\s+[·]\s+|\s+-\s+(?=[A-Za-z가-힣])/).filter(Boolean):[part];source.push(...pieces)}
      const lines=[];
      function addSegment(seg){seg=seg.replace(/\s+/g,' ').trim();if(!seg)return;let line='';const words=seg.includes(' ')?seg.split(' '):[...seg];const sep=seg.includes(' ')?' ':'';for(const word of words){const candidate=line?line+sep+word:word;if(approxWidth(candidate,size)<=maxWidth){line=candidate;continue}if(line){lines.push(line);if(lines.length>=maxLines)return;line=''}if(approxWidth(word,size)<=maxWidth){line=word;continue}let part='';for(const ch of [...word]){if(approxWidth(part+ch,size)>maxWidth&&part){lines.push(part);if(lines.length>=maxLines)return;part=ch}else part+=ch}line=part}if(line&&lines.length<maxLines)lines.push(line)}
      for(const seg of source){if(lines.length>=maxLines)break;addSegment(seg)}
      const full=source.join(' ');if(lines.length===maxLines&&approxWidth(lines.join(' '),size)<approxWidth(full,size)-2){let last=lines[maxLines-1];while(last&&approxWidth(last+'…',size)>maxWidth)last=last.slice(0,-1);lines[maxLines-1]=last+'…'}return lines
    }
    function finishPage(){if(!ops.length)return;text(M,20,`BLUE OCEAN MARKET - ${display(statement.title,true)}`,6);rightText(W-M,20,`Page ${pageNo}`,6);pages.push(ops.join('\n'));ops=[]}
    function pageHeader(first=false){pageNo++;y=752;text(M,y,'BLUE OCEAN MARKET',15,true);rightText(W-M,y,statement.title,10,true,true);y-=12;rule(y);y-=18;text(M,y,'Statement Period',7,true,true);text(M+88,y,`${statement.period.from||'Beginning'} - ${statement.period.to||'Current'}`,7);rightText(W-M,y,`${display('Generated',true)}: ${generated}`,6.5);y-=18;if(first&&statement.profile){text(M,y,'Profile Summary',9,true,true);y-=14;const entries=Object.entries(statement.profile).filter(([,v])=>v!==''&&v!=null);for(let i=0;i<entries.length;i+=2){const pair=entries.slice(i,i+2),lineSets=pair.map(([k,v])=>wrapWidth(v,175,6.3,3));const h=Math.max(1,...lineSets.map(x=>x.length))*9+4;pair.forEach(([k,v],j)=>{const x=M+j*272;text(x,y,k,6.2,true,true);const lines=lineSets[j];lines.forEach((line,idx)=>text(x+86,y-idx*8,line,6.2))});y-=h}y-=3}}
    function summary(){const entries=Object.entries(statement.summary||{}).filter(([,v])=>v!==''&&v!=null);if(!entries.length)return;text(M,y,'Period Summary',9,true,true);y-=10;for(let i=0;i<entries.length;i+=2){const pair=entries.slice(i,i+2);rect(M,y-19,W-2*M,24,.975,true);pair.forEach(([k,v],j)=>{const x=M+8+j*272;text(x,y-5,k,6.2,true,true);const shown=typeof v==='number'?`${statement.currency||''} ${money(v)}`:String(v);rightText(x+252,y-5,shown,7.2,true)});y-=29}y-=3}
    const cols={date:M,desc:M+58,ref:M+282,debitRight:M+421,creditRight:M+481,balanceRight:W-M-4};
    const widths={desc:218,ref:78};
    function tableHeader(){rect(M,y-4,W-2*M,18,.90,false);text(cols.date+3,y+1,'Date',6.5,true,true);text(cols.desc+3,y+1,'Description',6.5,true,true);text(cols.ref+3,y+1,'Reference',6.5,true,true);rightText(cols.debitRight,y+1,'Debit',6.5,true,true);rightText(cols.creditRight,y+1,'Credit',6.5,true,true);rightText(cols.balanceRight,y+1,'Balance',6.5,true,true);y-=21}
    ...
    for(const r of statement.rows||[]){
      const descLines=wrapWidth(r.description,widths.desc,6.2,5,{semantic:true});
      const machineLines=r.machine?wrapWidth((lang==='ko'?'장비: ':'Machine: ')+r.machine,widths.desc,5.4,3,{semantic:true}):[];
      const refLines=wrapWidth(r.reference,widths.ref,5.9,3);
      const lineCount=Math.max(1,descLines.length+machineLines.length,refLines.length),h=Math.max(22,13+lineCount*8.5);ensure(h);rule(y+5);const rowTop=y-5;
      text(cols.date+3,rowTop,dateOnly(r.date),6.0);
      descLines.forEach((line,i)=>text(cols.desc+3,rowTop-i*8.2,line,6.1,i===0));
      machineLines.forEach((line,i)=>text(cols.desc+3,rowTop-(descLines.length+i)*8.2,line,5.3));
      refLines.forEach((line,i)=>text(cols.ref+3,rowTop-i*8.2,line,5.8));
      rightText(cols.debitRight,rowTop,r.debit?money(r.debit):'',5.9);rightText(cols.creditRight,rowTop,r.credit?money(r.credit):'',5.9);rightText(cols.balanceRight,rowTop,money(r.balance),5.9,true);y-=h;
    }
    if(!(statement.rows||[]).length){ensure(24);text(M+8,y-5,'No transactions in the selected period.',7,false,true);y-=20}
    finishPage();
