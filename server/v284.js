...
function install({app,db,auth,allow,currentUnit,enforceUnit,isFinanceReviewer}){
  ...
  function statementPdf(statement,lang='ko'){
    ...
    for(const r of statement.rows||[]){const h=r.machine?27:18;ensure(h);rule(y+5);text(cols.date+3,y-5,dateOnly(r.date),6.1);text(cols.desc+3,y-5,String(r.description||'').slice(0,48),6.1);text(cols.ref+3,y-5,String(r.reference||'').slice(0,17),6.1);rightText(cols.credit-5,y-5,r.debit?money(r.debit):'',6.1);rightText(cols.balance-5,y-5,r.credit?money(r.credit):'',6.1);rightText(W-M-4,y-5,money(r.balance),6.1,true);y-=13;if(r.machine){text(cols.desc+3,y-4,String(r.machine).slice(0,82),5.5);y-=10}else y-=5}
