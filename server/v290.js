...
function install({app,db,auth,allow,currentUnit,enforceUnit,isFinanceReviewer,audit,notify,uploads,hasAccess,upload:sharedUpload}){
  ...
  app.get('/api/accounting/periods/close-readiness',auth,allow('finance'),(req,res)=>{
    ...
    processQueue(5000);res.json(periodCloseReadiness(bu,key));
  });
  ...
  app.post('/api/accounting/periods/close',auth,requireAccountingWrite,(req,res)=>{
    ...
    processQueue(5000);const readiness=periodCloseReadiness(bu,key);if(!readiness.ready)return res.status(409).json({error:'Period close is blocked until reconciliation and accounting controls are complete.',blockers:readiness.blockers,readiness});
