...
function install({app,db,auth,allow,currentUnit,enforceUnit,isFinanceReviewer,audit,notify,uploads,hasAccess,upload:sharedUpload}){
  ...
  CREATE TABLE IF NOT EXISTS accounting_journal_documents(
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    journal_entry_id INTEGER NOT NULL,
    title TEXT NOT NULL DEFAULT 'Accounting Evidence',
  ...
  CREATE INDEX IF NOT EXISTS idx_accounting_journal_documents_journal ON accounting_journal_documents(journal_entry_id,id);
  ...
  CREATE TABLE IF NOT EXISTS accounting_period_reconciliations(
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    business_unit_id INTEGER,
    period_key TEXT NOT NULL,
    ...
    completed_at TEXT,
    updated_at TEXT DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(business_unit_id,period_key,area_key),
    FOREIGN KEY(business_unit_id) REFERENCES business_units(id),
    FOREIGN KEY(completed_by) REFERENCES users(id)
  );
  CREATE INDEX IF NOT EXISTS idx_period_recon_scope ON accounting_period_reconciliations(period_key,business_unit_id,status);
  CREATE UNIQUE INDEX IF NOT EXISTS idx_period_recon_unique_scope ON accounting_period_reconciliations(COALESCE(business_unit_id,0),period_key,area_key);
  ...
  const PERIOD_RECON_CORE=[
    ['bank','Bank Reconciliation'],['cash','Cash Count / Cash Book'],['payroll','Payroll'],['tax','Tax / WHT / Duties'],
    ['inter_bu','Inter-BU Clearing'],['suspense','Suspense / Clearing'],['corrections','Corrections / Voids'],['trial_balance','Trial Balance']
  ];
  const PERIOD_RECON_EXCAVATOR=[
    ['exc_supplier_payables','Excavator Supplier Payables'],['exc_supplier_credit','Excavator Supplier Credit'],
    ['exc_buyer_advances','Excavator Buyer Advances'],['exc_inventory','Excavator Inventory'],['exc_resale_share','Pakistan Resale Profit Share']
  ];
  const PERIOD_RECON_PINK_SALT=[
    ['ps_supplier_payables','Pink Salt Supplier Payables'],['ps_supplier_credit','Pink Salt Supplier Shared Credit'],
    ['ps_goods_in_transit','Pink Salt Goods in Transit'],['ps_raw_inventory','Raw Salt Inventory'],
    ['ps_packaging_inventory','Packaging Inventory'],['ps_wip','Work in Process'],['ps_finished_goods','Finished Goods'],
    ['ps_customer_balances','Customer Receivables / Advances']
  ];
  function periodReconAreas(businessUnitId){
    let rows=[...PERIOD_RECON_CORE];
    if(!businessUnitId)return [...rows,['bu_close','Business Unit Close Completion']];
    const b=db.prepare('SELECT name FROM business_units WHERE id=?').get(Number(businessUnitId)),name=text(b?.name).toLowerCase();
    if(name.includes('excavator'))rows=rows.concat(PERIOD_RECON_EXCAVATOR);
    if(name.includes('pink')&&name.includes('salt')||name==='pink salt')rows=rows.concat(PERIOD_RECON_PINK_SALT);
    if(!name.includes('excavator')&&!(name.includes('pink')&&name.includes('salt')))rows.push(['operational_subledgers','Operational Subledgers / Inventory']);
    return rows;
  }
  function ensurePeriodReconRows(businessUnitId,key){
    const ins=db.prepare(`INSERT OR IGNORE INTO accounting_period_reconciliations(business_unit_id,period_key,area_key,area_name,status) VALUES(?,?,?,?,'Open')`);
    for(const [areaKey,areaName] of periodReconAreas(businessUnitId))ins.run(businessUnitId||null,key,areaKey,areaName);
  }
  function periodJournalScope(businessUnitId,alias='j'){
    if(!businessUnitId)return {sql:'',args:[]};
    return {sql:` AND (${alias}.business_unit_id=? OR EXISTS(SELECT 1 FROM accounting_journal_lines _pcu WHERE _pcu.journal_entry_id=${alias}.id AND _pcu.business_unit_id=?))`,args:[Number(businessUnitId),Number(businessUnitId)]};
  }
  function periodCloseReadiness(businessUnitId,key){
    const bu=Number(businessUnitId||0)||null;ensurePeriodReconRows(bu,key);const blockers=[],checks=[];
    const add=(key0,label,count,detail,hard=true)=>{const n=Number(count||0),ok=n===0;checks.push({key:key0,label,count:n,ok,detail:detail||''});if(hard&&!ok)blockers.push(`${label}: ${n}${detail?' · '+detail:''}`)};
    const js=periodJournalScope(bu,'j');
    const pending=db.prepare(`SELECT COUNT(*) c FROM accounting_journal_entries j WHERE j.status IN ('Pending Review','Correction Required') AND substr(j.transaction_date,1,7)=?${js.sql}`).get(key,...js.args).c;
    add('posting_control','Posting Control proposals waiting',pending,'finalize or correct before close');
    const sync=bu?db.prepare("SELECT COUNT(*) c FROM accounting_sync_queue q JOIN finance_entries f ON f.id=q.finance_entry_id WHERE f.business_unit_id=? AND substr(COALESCE(f.transaction_date,f.created_at),1,7)=?").get(bu,key).c:db.prepare("SELECT COUNT(*) c FROM accounting_sync_queue q JOIN finance_entries f ON f.id=q.finance_entry_id WHERE substr(COALESCE(f.transaction_date,f.created_at),1,7)=?").get(key).c;
    add('sync_queue','Finance transactions waiting for accounting synchronization',sync,'resolve synchronization before close');
    const exc=bu?db.prepare("SELECT COUNT(*) c FROM accounting_exceptions WHERE status='Open' AND business_unit_id=?").get(bu).c:db.prepare("SELECT COUNT(*) c FROM accounting_exceptions WHERE status='Open'").get().c;
    add('exceptions','Open accounting exceptions',exc,'resolve or formally clear exceptions');
    const unverified=bu?db.prepare("SELECT COUNT(*) c FROM finance_entries WHERE business_unit_id=? AND status!='Voided' AND (COALESCE(cash_effect,0)<>0 OR source_type='Manual') AND substr(COALESCE(transaction_date,created_at),1,7)=? AND COALESCE(verification_status,'') NOT IN ('Verified','Verified / Correct')").get(bu,key).c:db.prepare("SELECT COUNT(*) c FROM finance_entries WHERE status!='Voided' AND (COALESCE(cash_effect,0)<>0 OR source_type='Manual') AND substr(COALESCE(transaction_date,created_at),1,7)=? AND COALESCE(verification_status,'') NOT IN ('Verified','Verified / Correct')").get(key).c;
    add('finance_verification','Unverified Finance transactions',unverified,'verify or resolve corrections before close');
    const openCorrections=bu?db.prepare("SELECT COUNT(*) c FROM finance_correction_requests c JOIN finance_entries f ON f.id=c.finance_entry_id WHERE c.business_unit_id=? AND c.status IN ('Open','Viewed','Resubmitted') AND substr(COALESCE(f.transaction_date,f.created_at),1,7)=?").get(bu,key).c:db.prepare("SELECT COUNT(*) c FROM finance_correction_requests c JOIN finance_entries f ON f.id=c.finance_entry_id WHERE c.status IN ('Open','Viewed','Resubmitted') AND substr(COALESCE(f.transaction_date,f.created_at),1,7)=?").get(key).c;
    add('finance_corrections','Open Finance correction requests',openCorrections,'no unresolved correction may cross period close');
    const unreconciled=bu?db.prepare("SELECT COUNT(*) c FROM accounting_bank_statement_lines s JOIN accounting_payment_accounts p ON p.id=s.payment_account_id WHERE s.status!='Reconciled' AND substr(s.statement_date,1,7)=? AND (p.business_unit_id=? OR p.business_unit_id IS NULL)").get(key,bu).c:db.prepare("SELECT COUNT(*) c FROM accounting_bank_statement_lines s WHERE s.status!='Reconciled' AND substr(s.statement_date,1,7)=?").get(key).c;
    add('bank_lines','Unreconciled bank statement lines',unreconciled,'match or formally resolve all imported statement lines');
    const unbalanced=db.prepare(`SELECT COUNT(*) c FROM (SELECT j.id,ROUND(COALESCE(SUM(l.debit_krw),0)-COALESCE(SUM(l.credit_krw),0),2) diff FROM accounting_journal_entries j JOIN accounting_journal_lines l ON l.journal_entry_id=j.id WHERE j.status IN ('Posted','Reversed') AND substr(j.transaction_date,1,7)=?${js.sql} GROUP BY j.id HAVING ABS(diff)>0.01)`).get(key,...js.args).c;
    add('trial_balance','Unbalanced official journals',unbalanced,'hard accounting integrity block');
    const suspenseArgs=[key],suspenseScope=bu?' AND l.business_unit_id=?':'';if(bu)suspenseArgs.push(bu);
    const suspense=db.prepare(`SELECT COUNT(*) c FROM (SELECT a.id,ROUND(COALESCE(SUM(l.debit_krw-l.credit_krw),0),2) bal FROM accounting_accounts a JOIN accounting_journal_lines l ON l.account_id=a.id JOIN accounting_journal_entries j ON j.id=l.journal_entry_id WHERE j.status IN ('Posted','Reversed') AND substr(j.transaction_date,1,7)<=?${suspenseScope} AND (a.subtype='Suspense' OR a.system_key IN ('SUSPENSE_ASSET','SUSPENSE_LIABILITY')) GROUP BY a.id HAVING ABS(bal)>0.01)`).get(...suspenseArgs).c;
    add('suspense','Suspense / temporary clearing balances',suspense,'target zero before close');
    const inter=db.prepare(`SELECT
      COALESCE(SUM(CASE WHEN a.system_key='INTER_BU_RECEIVABLE' THEN l.debit_krw-l.credit_krw ELSE 0 END),0) due_from,
      COALESCE(SUM(CASE WHEN a.system_key='INTER_BU_PAYABLE' THEN l.credit_krw-l.debit_krw ELSE 0 END),0) due_to
      FROM accounting_journal_lines l JOIN accounting_journal_entries j ON j.id=l.journal_entry_id JOIN accounting_accounts a ON a.id=l.account_id
      WHERE j.status IN ('Posted','Reversed') AND substr(j.transaction_date,1,7)<=? AND a.system_key IN ('INTER_BU_RECEIVABLE','INTER_BU_PAYABLE')`).get(key);
    const interDiff=Math.abs(num(inter?.due_from)-num(inter?.due_to));checks.push({key:'inter_bu_balance',label:'Inter-BU Due From / Due To difference',count:interDiff,ok:interDiff<=0.01,detail:`Due From ${num(inter?.due_from).toFixed(2)} · Due To ${num(inter?.due_to).toFixed(2)}`});if(interDiff>0.01)blockers.push(`Inter-BU Clearing mismatch: ${interDiff.toFixed(2)} KRW`);
    if(!bu){const openBus=db.prepare(`SELECT COUNT(*) c FROM business_units b LEFT JOIN accounting_periods p ON p.business_unit_id=b.id AND p.period_key=? AND p.status='Closed' WHERE b.status!='Archived' AND p.id IS NULL`).get(key).c;add('business_units_closed','Business units not yet closed',openBus,'close each active BU before company consolidation close')}
    const checklist=db.prepare(`SELECT r.*,u.name completed_by_name FROM accounting_period_reconciliations r LEFT JOIN users u ON u.id=r.completed_by WHERE r.period_key=? AND COALESCE(r.business_unit_id,0)=COALESCE(?,0) ORDER BY r.id`).all(key,bu);
    const incomplete=checklist.filter(r=>!['Reconciled','Exception Approved','Not Applicable'].includes(r.status));
    add('reconciliation_signoff','Required reconciliation checklist items still Open',incomplete.length,'Finance/Accounting sign-off is mandatory for each applicable V2 reconciliation area');
    return {ready:blockers.length===0,business_unit_id:bu,period_key:key,checks,checklist,blockers};
  }
  ...
  function buildFinancePosting(finance){
    ...
    if(st==='Excavator Buyer Payment'||st==='Buyer Payment'){
      ...
      if(saleSpecific){
        // V30.26: a sale-time receipt can be larger than the sale outstanding. Only the amount
        // actively allocated to its originating sale clears A/R; the excess is a Buyer Advance.
        let saleAssetId=0,directAllocated=0;
        if(text(p.source_type)==='Excavator Sale'&&Number(p.source_id||0)){try{saleAssetId=Number(db.prepare('SELECT asset_id FROM excavator_transactions WHERE id=?').get(Number(p.source_id))?.asset_id||0)}catch(_){}}
        if(saleAssetId){try{directAllocated=Number(db.prepare(`SELECT COALESCE(SUM(amount_krw),0) v FROM excavator_buyer_payment_allocations WHERE payment_id=? AND asset_id=? AND COALESCE(status,'Active')='Active'`).get(p.id,saleAssetId)?.v||0)}catch(_){}}
        if(!saleAssetId)directAllocated=Math.min(amount,Math.max(0,Number(p.krw_amount||amount)));
        directAllocated=Math.min(amount,Math.max(0,directAllocated));const advanceCredit=Math.max(0,amount-directAllocated);
        if(directAllocated>0.005)credit(accountId('ACCOUNTS_RECEIVABLE'),directAllocated,{...entityBase,entity_type:'Buyer',entity_id:p.buyer_id,memo:'Sale payment clearing'});
        if(advanceCredit>0.005)credit(accountId('CUSTOMER_ADVANCES'),advanceCredit,{...entityBase,entity_type:'Buyer',entity_id:p.buyer_id,memo:'Excess buyer receipt / unallocated credit'});
      ...
      // V30.26: recognize the settlement by its actual active allocation sources. A sale may be
      // covered partly by an existing buyer advance and partly by a newly received payment.
      // Treating the whole mixed settlement as A/R leaves Customer Advances wrong.
      let settlement={advance_krw:0,direct_krw:0};
      try{settlement=db.prepare(`SELECT
        COALESCE(SUM(CASE WHEN COALESCE(p.source_type,'')='Excavator Sale' OR COALESCE(p.payment_type,'')='Sale Payment' THEN 0 ELSE al.amount_krw END),0) advance_krw,
        COALESCE(SUM(CASE WHEN COALESCE(p.source_type,'')='Excavator Sale' OR COALESCE(p.payment_type,'')='Sale Payment' THEN al.amount_krw ELSE 0 END),0) direct_krw
        FROM excavator_buyer_payment_allocations al
        JOIN excavator_buyer_payments p ON p.id=al.payment_id
        WHERE al.asset_id=? AND COALESCE(al.status,'Active')='Active' AND COALESCE(p.status,'Active')!='Voided'`).get(sale.asset_id)||settlement}catch(_){}
      let advanceAllocated=Math.max(0,Number(settlement.advance_krw||0)),directAllocated=Math.max(0,Number(settlement.direct_krw||0));
      const activeAllocated=advanceAllocated+directAllocated;
      if(activeAllocated<=0.005){if(paymentSource.includes('advance'))advanceAllocated=amount;else directAllocated=amount;}
      else{
        const scale=activeAllocated>amount+0.005?amount/activeAllocated:1;advanceAllocated*=scale;directAllocated*=scale;
        const remainder=Math.max(0,amount-advanceAllocated-directAllocated);directAllocated+=remainder;
      }
      if(advanceAllocated>0.005)debit(accountId('CUSTOMER_ADVANCES'),advanceAllocated,{...entityBase,entity_type:'Buyer',entity_id:buyerId,memo:'Apply buyer advance to sale'});
      if(directAllocated>0.005)debit(accountId('ACCOUNTS_RECEIVABLE'),directAllocated,{...entityBase,entity_type:'Buyer',entity_id:buyerId,memo:'Recognize sale receivable / direct settlement'});
      ...
      const cost=excavatorCostSnapshot(sale.asset_id),cogs=Math.max(0,Number(cost?.total||0));if(cogs>0){debit(accountId('EXCAVATOR_COGS'),cogs,{business_unit_id:finance.business_unit_id,entity_type:'Machine',entity_id:sale.asset_id,memo:'Recognize machine cost on sale'});credit(accountId('EXCAVATOR_INVENTORY'),cogs,{business_unit_id:finance.business_unit_id,entity_type:'Machine',entity_id:sale.asset_id,memo:'Remove sold machine from inventory'});}ctx.asset_id=sale.asset_id;ctx.buyer_id=buyerId;ctx.cost=cogs;ctx.payment_source=paymentSource;ctx.advance_settlement_krw=advanceAllocated;ctx.direct_settlement_krw=directAllocated;
      ...
      // V30.25 operational recognition only. Cash settlement is a separate Sale Payment Finance event.
      const s=db.prepare('SELECT * FROM sales WHERE id=?').get(finance.source_id);if(!s)return {error:'Linked sale was not found.'};
      debit(accountId('ACCOUNTS_RECEIVABLE'),amount,{...entityBase,entity_type:'Sale',entity_id:s.id,memo:'Recognize customer receivable'});credit(accountId('GENERAL_SALES_REVENUE'),amount,{...entityBase,entity_type:'Sale',entity_id:s.id,memo:'Sales revenue'});const cost=Math.max(0,Number(s.cost_total||0));if(cost){debit(accountId('GENERAL_COGS'),cost,{business_unit_id:finance.business_unit_id,entity_type:'Sale',entity_id:s.id,memo:'Cost of goods sold'});credit(accountId('GENERAL_INVENTORY'),cost,{business_unit_id:finance.business_unit_id,entity_type:'Sale',entity_id:s.id,memo:'Inventory released'});}ctx.sale_id=s.id;
    }else if(st==='Sale Payment'||st==='MIMI Sale Payment'){
      const s=db.prepare('SELECT * FROM sales WHERE id=?').get(finance.source_id);if(!s)return {error:'Linked sale payment source was not found.'};debit(cash,amount,{...entityBase,entity_type:'Sale',entity_id:s.id,memo:'Customer payment received'});credit(accountId('ACCOUNTS_RECEIVABLE'),amount,{...entityBase,entity_type:'Sale',entity_id:s.id,memo:'Settle sale receivable'});ctx.sale_id=s.id;
    }else if(st==='Purchase'){
      // V30.25 operational purchase recognition only. Real payment is posted separately.
      debit(accountId('GENERAL_INVENTORY'),amount,{...entityBase,entity_type:'Purchase',entity_id:finance.source_id,memo:'Inventory purchase / cost recognition'});credit(accountId('ACCOUNTS_PAYABLE'),amount,{...entityBase,entity_type:'Purchase',entity_id:finance.source_id,memo:'Supplier payable recognized'});ctx.purchase_id=finance.source_id;
    }else if(st==='Purchase Payment'){
      const p=db.prepare('SELECT * FROM purchases WHERE id=?').get(finance.source_id);if(!p)return {error:'Linked purchase payment source was not found.'};debit(accountId('ACCOUNTS_PAYABLE'),amount,{...entityBase,entity_type:'Purchase',entity_id:p.id,memo:'Settle supplier payable'});credit(cash,amount,{...entityBase,entity_type:'Purchase',entity_id:p.id,memo:'Supplier payment'});ctx.purchase_id=p.id;
    }else{
  ...
  function reverseJournal(entry,reason='Source changed or voided',userId=null){
    if(!entry||entry.status==='Reversed'||entry.status==='Cancelled')return null;
    // A proposal that has not reached the official ledger can be cancelled without
    // creating a financial reversal. Only a Posted journal has a ledger effect to reverse.
    if(['Pending Review','Correction Required'].includes(entry.status)){db.prepare("UPDATE accounting_journal_entries SET status='Cancelled',finance_entry_id=NULL,description=COALESCE(description,'')||?,updated_at=CURRENT_TIMESTAMP WHERE id=?").run(` | Cancelled before posting: ${reason}`,entry.id);return null;}
    const lines=db.prepare('SELECT * FROM accounting_journal_lines WHERE journal_entry_id=? ORDER BY id').all(entry.id),jNo=numbering.next('accounting.reversal',{businessUnitId:entry.business_unit_id});
    // V30.18: reversing an official Posted journal is itself a controlled accounting mutation.
    // Create a reversal proposal and keep the original Posted until an authorized reviewer
    // final-posts the reversal in Posting Control. This prevents silent GL changes.
    const r=db.prepare(`INSERT INTO accounting_journal_entries(journal_no,business_unit_id,transaction_date,source_type,source_id,source_label,description,status,reversal_of_id,created_by) VALUES(?,?,?,?,?,?,?,'Pending Review',?,?)`)
      .run(jNo,entry.business_unit_id,nowDate(),'Accounting Reversal',entry.id,'Controlled Reversal Proposal',reason,entry.id,userId||entry.created_by||null);
    ...
    for(const l of lines)ins.run(rid,l.account_id,l.business_unit_id,l.credit_krw,l.debit_krw,l.original_amount,l.original_currency,l.fx_rate,l.entity_type,l.entity_id,l.dimension_json,`Reversal proposal · ${l.memo||''}`);
    // Release the unique Finance link so a corrected Finance proposal can be prepared while
    // retaining the original source_id for traceability. The original remains Posted.
    db.prepare("UPDATE accounting_journal_entries SET finance_entry_id=NULL,updated_at=CURRENT_TIMESTAMP WHERE id=?").run(entry.id);
    try{db.prepare("UPDATE accounting_journal_entries SET submitted_for_review_at=CURRENT_TIMESTAMP WHERE id=?").run(rid)}catch(_){ }
  ...
  function postJournal({businessUnitId,transactionDate,sourceType,sourceId,sourceLabel,description,financeEntryId=null,sourceHash='',createdBy=null,lines,postingStatus='Pending Review'}){
    ...
    if(status==='Pending Review'){
      try{db.prepare("UPDATE accounting_journal_entries SET submitted_for_review_at=CURRENT_TIMESTAMP WHERE id=?").run(jid)}catch(_){ }
      try{db.prepare("INSERT INTO accounting_posting_history(journal_entry_id,business_unit_id,user_id,action,note,before_status,after_status) VALUES(?,?,?,?,?,'','Pending Review')").run(jid,businessUnitId||null,createdBy||null,'Submitted for Posting Review',description||sourceLabel||sourceType)}catch(_){ }
    }
    return jid;
  }
  ...
  function syncFinanceEntry(financeId){
    ...
    if(text(finance.status)==='Voided'){
      ...
      if(reversalId)db.prepare("UPDATE finance_entries SET accounting_status='Pending Review',accounting_journal_id=? WHERE id=?").run(reversalId,finance.id);else db.prepare("UPDATE finance_entries SET accounting_status='Reversed',accounting_journal_id=NULL WHERE id=?").run(finance.id);
      db.prepare('DELETE FROM accounting_sync_queue WHERE finance_entry_id=?').run(finance.id);return reversalId?{reversal_pending:true,journal_id:reversalId}:{reversed:true};
    }
  ...
  // V30.39.2: Accounting synchronization is deliberately time-sliced. better-sqlite3 is
  // synchronous, so processing hundreds/thousands of rows inside an interactive request can
  // stall every user. Keep each slice small and bounded; business mutations still enqueue
  // immediately and Accounting catches up continuously in the background.
  function processQueue(limit=40,timeBudgetMs=25){
    if(processing)return {busy:true,processed:0,remaining:Number(db.prepare('SELECT COUNT(*) c FROM accounting_sync_queue').get()?.c||0)};
    processing=true;let processed=0;const started=Date.now(),hardLimit=Math.max(1,Math.min(Number(limit)||40,100)),budget=Math.max(5,Math.min(Number(timeBudgetMs)||25,50));
    try{
      const rows=db.prepare('SELECT * FROM accounting_sync_queue ORDER BY queued_at LIMIT ?').all(hardLimit);
      for(const q of rows){
        if(processed>0&&Date.now()-started>=budget)break;
        try{syncFinanceEntry(q.finance_entry_id)}catch(e){
          db.prepare("UPDATE accounting_sync_queue SET attempts=attempts+1,last_error=?,queued_at=CURRENT_TIMESTAMP WHERE finance_entry_id=?").run(String(e.message||e).slice(0,1000),q.finance_entry_id);
          const f=db.prepare('SELECT * FROM finance_entries WHERE id=?').get(q.finance_entry_id);
          if(f){db.prepare("UPDATE finance_entries SET accounting_status='Error' WHERE id=?").run(f.id);addException(f,'ACCOUNTING_SYNC_ERROR',String(e.message||e),'Critical')}
        }
        processed++;
      }
    }finally{processing=false}
    return {busy:false,processed,remaining:Number(db.prepare('SELECT COUNT(*) c FROM accounting_sync_queue').get()?.c||0),elapsed_ms:Date.now()-started};
  }
  function scheduleQueue(delay=250){
    if(queueTimer)return;
    queueTimer=setTimeout(()=>{queueTimer=null;try{const r=processQueue(40,25);scheduleQueue(r?.remaining?100:750)}catch(e){console.error('Accounting background sync:',e.message);scheduleQueue(1500)}},Math.max(0,Number(delay)||0));
    if(typeof queueTimer.unref==='function')queueTimer.unref();
  }

  // Historic Finance backfill is a one-time migration, not work to repeat on every restart.
  ...
  const migrationKey='v30.39.2-accounting-initial-queue';
  if(!db.prepare('SELECT 1 FROM system_migrations WHERE migration_key=?').get(migrationKey)){
    const r=db.prepare("INSERT OR IGNORE INTO accounting_sync_queue(finance_entry_id,queued_at) SELECT id,CURRENT_TIMESTAMP FROM finance_entries").run();
    db.prepare('INSERT OR REPLACE INTO system_migrations(migration_key,applied_at,details) VALUES(?,CURRENT_TIMESTAMP,?)').run(migrationKey,`Queued ${Number(r.changes||0)} historic Finance record(s) once.`);
  }
  scheduleQueue(150);
  ...
  function requireAccountingWrite(req,res,next){const bu=currentUnit(req);if(req.user.role==='CEO / Owner'||hasAccess?.(req.user.id,'sensitive.accounting_adjustments',bu))return next();return res.status(403).json({error:'Accounting adjustment permission is required for this action.'})}
  function requirePayroll(req,res,next){const bu=currentUnit(req);if(req.user.role==='CEO / Owner'||hasAccess?.(req.user.id,'sensitive.payroll',bu))return next();return res.status(403).json({error:'Payroll access is not authorized for your account.'})}
  ...
  app.get('/api/accounting/overview',auth,allow('finance','dashboard'),(req,res)=>{
    const f=lineUnitFilter(req,'l'),args=f.args;
    const totals=db.prepare(`SELECT COALESCE(SUM(l.debit_krw),0) debit,COALESCE(SUM(l.credit_krw),0) credit FROM accounting_journal_lines l JOIN accounting_journal_entries j ON j.id=l.journal_entry_id WHERE j.status IN ('Posted','Reversed')${f.sql}`).get(...args);
    const byType=db.prepare(`SELECT a.account_type,COALESCE(SUM(l.debit_krw-l.credit_krw),0) net FROM accounting_journal_lines l JOIN accounting_journal_entries j ON j.id=l.journal_entry_id JOIN accounting_accounts a ON a.id=l.account_id WHERE j.status IN ('Posted','Reversed')${f.sql} GROUP BY a.account_type`).all(...args);
  ...
  app.get('/api/accounting/journal',auth,allow('finance','dashboard'),(req,res)=>{const f=journalUnitFilter(req,'j'),from=text(req.query.from),to=text(req.query.to),accountIdQ=Number(req.query.account_id||0);let q=`SELECT j.*,b.name business_unit,u.name created_by_name,(SELECT ROUND(SUM(debit_krw),2) FROM accounting_journal_lines WHERE journal_entry_id=j.id) total_debit,(SELECT ROUND(SUM(credit_krw),2) FROM accounting_journal_lines WHERE journal_entry_id=j.id) total_credit FROM accounting_journal_entries j LEFT JOIN business_units b ON b.id=j.business_unit_id LEFT JOIN users u ON u.id=j.created_by WHERE 1=1${f.sql}`,args=[...f.args];if(from){q+=' AND date(j.transaction_date)>=date(?)';args.push(from)}if(to){q+=' AND date(j.transaction_date)<=date(?)';args.push(to)}if(accountIdQ){q+=' AND EXISTS(SELECT 1 FROM accounting_journal_lines x WHERE x.journal_entry_id=j.id AND x.account_id=?)';args.push(accountIdQ)}q+=' ORDER BY date(j.transaction_date) DESC,j.id DESC LIMIT 1000';res.json(db.prepare(q).all(...args))});
   // V30.49 deterministic GL paging: original /journal array and /journal/:id stay unchanged.
   app.get('/api/v349/accounting/journal/page',auth,allow('finance','dashboard'),(req,res)=>{try{
     const f=journalUnitFilter(req,'j'),from=text(req.query.from),to=text(req.query.to),search=text(req.query.search).slice(0,100),accountId=Number(req.query.account_id||0),size=[25,50,100].includes(Number(req.query.pageSize))?Number(req.query.pageSize):25;
     let where='1=1'+f.sql,args=[...f.args];
     if(from){where+=' AND date(j.transaction_date)>=date(?)';args.push(from)}if(to){where+=' AND date(j.transaction_date)<=date(?)';args.push(to)}
     if(accountId>0&&Number.isSafeInteger(accountId)){where+=' AND EXISTS(SELECT 1 FROM accounting_journal_lines x WHERE x.journal_entry_id=j.id AND x.account_id=?)';args.push(accountId)}
     if(search){where+=' AND (j.journal_no LIKE ? OR j.source_label LIKE ? OR j.description LIKE ?)';args.push(...Array(3).fill('%'+search+'%'))}
     const total=db.prepare(`SELECT COUNT(*) n FROM accounting_journal_entries j WHERE ${where}`).get(...args).n,pages=Math.max(1,Math.ceil(total/size)),page=Math.min(pages,Math.max(1,Math.trunc(Number(req.query.page)||1)));
     const rows=db.prepare(`WITH page AS (SELECT j.* FROM accounting_journal_entries j WHERE ${where} ORDER BY date(j.transaction_date) DESC,j.id DESC LIMIT ? OFFSET ?),
       amounts AS (SELECT l.journal_entry_id,ROUND(SUM(l.debit_krw),2) total_debit,ROUND(SUM(l.credit_krw),2) total_credit FROM accounting_journal_lines l JOIN page p ON p.id=l.journal_entry_id GROUP BY l.journal_entry_id)
       SELECT p.*,b.name business_unit,u.name created_by_name,a.total_debit,a.total_credit FROM page p LEFT JOIN business_units b ON b.id=p.business_unit_id LEFT JOIN users u ON u.id=p.created_by LEFT JOIN amounts a ON a.journal_entry_id=p.id ORDER BY date(p.transaction_date) DESC,p.id DESC`).all(...args,size,(page-1)*size);
     res.json({rows,pagination:{page,pageSize:size,page_size:size,pages,total,from:total?(page-1)*size+1:0,to:Math.min(total,page*size)}});
   }catch(e){res.status(500).json({error:'Could not load journal page.'})}});
  app.get('/api/accounting/journal/:id',auth,allow('finance','dashboard'),(req,res)=>{const j=db.prepare('SELECT j.*,b.name business_unit,u.name created_by_name FROM accounting_journal_entries j LEFT JOIN business_units b ON b.id=j.business_unit_id LEFT JOIN users u ON u.id=j.created_by WHERE j.id=?').get(req.params.id);if(!j||!canSeeJournal(req,j.id,j.business_unit_id))return res.status(404).json({error:'Journal entry not found'});const lines=db.prepare('SELECT l.*,a.code,a.name account_name,a.account_type,b.name business_unit FROM accounting_journal_lines l JOIN accounting_accounts a ON a.id=l.account_id LEFT JOIN business_units b ON b.id=l.business_unit_id WHERE l.journal_entry_id=? ORDER BY l.id').all(j.id),documents=db.prepare('SELECT * FROM accounting_journal_documents WHERE journal_entry_id=? ORDER BY id').all(j.id);res.json({journal:j,lines,documents})});
  app.post('/api/accounting/journal',auth,requireAccountingWrite,upload.single('evidence'),(req,res)=>{
    const cleanup=()=>{try{if(req.file?.path)require('fs').unlinkSync(req.file.path)}catch(_){}};
    let bu;try{bu=writableBusinessUnit(req,req.body)}catch(e){cleanup();return res.status(400).json({error:e.message})}
    const date=dateOnly(req.body.transaction_date);if(!date){cleanup();return res.status(400).json({error:'Transaction date is required.'})}if(isPeriodClosed(bu,date)){cleanup();return res.status(409).json({error:'This accounting period is closed.'})}
    let lines=[];try{lines=Array.isArray(req.body.lines)?req.body.lines:JSON.parse(req.body.lines_json||'[]')}catch(_){cleanup();return res.status(400).json({error:'Manual journal lines are invalid.'})}
    if(!Array.isArray(lines)||lines.length<2){cleanup();return res.status(400).json({error:'At least two journal lines are required.'})}
    const normalized=[];for(const l of lines){const a=db.prepare('SELECT * FROM accounting_accounts WHERE id=? AND active=1').get(Number(l.account_id));if(!a||!a.allow_manual){cleanup();return res.status(400).json({error:'One or more selected accounts do not allow manual posting.'})}const debit=num(l.debit_krw),credit=num(l.credit_krw);if(debit<0||credit<0||(debit>0&&credit>0)||(!(debit>0)&&!(credit>0))){cleanup();return res.status(400).json({error:'Each journal line must contain either a debit or a credit greater than zero, not both.'})}normalized.push({account_id:a.id,business_unit_id:bu,debit_krw:debit,credit_krw:credit,memo:text(l.memo)})}
    const dr=normalized.reduce((n,l)=>n+l.debit_krw,0),cr=normalized.reduce((n,l)=>n+l.credit_krw,0);if(dr<=0||Math.abs(dr-cr)>0.005){cleanup();return res.status(400).json({error:'Manual journal must have equal debit and credit totals.'})}
    const reason=text(req.body.reason||req.body.description);if(!reason){cleanup();return res.status(400).json({error:'A reason/description is required for a manual journal proposal.'})}if(!req.file){cleanup();return res.status(400).json({error:'Evidence / supporting document is required for a manual journal proposal.'})}
    try{const tx=db.transaction(()=>{const jid=postJournal({businessUnitId:bu,transactionDate:date,sourceType:'Manual Journal',sourceId:null,sourceLabel:'Manual Journal',description:reason,createdBy:req.user.id,lines:normalized});db.prepare('INSERT INTO accounting_journal_documents(journal_entry_id,title,original_name,file_path,mime_type,size_bytes,uploaded_by) VALUES(?,?,?,?,?,?,?)').run(jid,'Manual Journal Evidence',req.file.originalname,'/uploads/'+req.file.filename,req.file.mimetype,req.file.size,req.user.id);audit(req.user,'accounting_journal',jid,'manual-submit-for-review',JSON.stringify({debit:dr,credit:cr,reason,evidence:req.file.originalname}));return jid});const jid=tx();res.json({id:jid,status:'Pending Review'})}catch(e){cleanup();return res.status(409).json({error:e.message||'Manual journal proposal could not be submitted.'})}
  });

  function reportRows(req,kind){
    const f=lineUnitFilter(req,'l'),from=text(req.query.from),to=text(req.query.to),args=[...f.args];let dateSql='';if(from){dateSql+=' AND date(j.transaction_date)>=date(?)';args.push(from)}if(to){dateSql+=' AND date(j.transaction_date)<=date(?)';args.push(to)}
  ...
  app.post('/api/accounting/resync',auth,requireAccountingWrite,(req,res)=>{let q='INSERT OR IGNORE INTO accounting_sync_queue(finance_entry_id,queued_at) SELECT id,CURRENT_TIMESTAMP FROM finance_entries WHERE 1=1',args=[];if(req.selected_business_unit_id){q+=' AND business_unit_id=?';args.push(req.selected_business_unit_id)}else if(req.user.role!=='CEO / Owner'){q+=' AND business_unit_id=?';args.push(req.user.business_unit_id)}db.prepare(q).run(...args);scheduleQueue(0);res.json({ok:true,queued:true,remaining:db.prepare('SELECT COUNT(*) c FROM accounting_sync_queue').get().c})});
  ...
  app.get('/api/accounting/periods/close-readiness',auth,allow('finance'),(req,res)=>{
    const key=text(req.query.period_key);if(!/^\d{4}-\d{2}$/.test(key))return res.status(400).json({error:'Period must be YYYY-MM.'});
    const companyScope=String(req.query.company_scope||'')==='1';let bu=companyScope?null:(req.query.business_unit_id?Number(req.query.business_unit_id):currentUnit(req));
    if(req.user.role!=='CEO / Owner'&&!bu)bu=req.user.business_unit_id;if(companyScope&&req.user.role!=='CEO / Owner')return res.status(403).json({error:'Only CEO / Owner can review consolidated company close readiness.'});if(bu&&!enforceUnit(req,bu))return res.status(403).json({error:'You cannot review another business unit period.'});
    res.json(periodCloseReadiness(bu,key));
  });
  app.post('/api/accounting/periods/reconciliation-status',auth,requireAccountingWrite,upload.single('evidence'),(req,res)=>{
    const cleanup=()=>{try{if(req.file?.path)require('fs').unlinkSync(req.file.path)}catch(_){}};const key=text(req.body.period_key),companyScope=String(req.body.company_scope||'')==='1';if(!/^\d{4}-\d{2}$/.test(key)){cleanup();return res.status(400).json({error:'Period must be YYYY-MM.'})}
    let bu=companyScope?null:(req.body.business_unit_id?Number(req.body.business_unit_id):currentUnit(req));if(req.user.role!=='CEO / Owner'&&!bu)bu=req.user.business_unit_id;if(companyScope&&req.user.role!=='CEO / Owner'){cleanup();return res.status(403).json({error:'Only CEO / Owner can sign off consolidated reconciliation.'})}if(bu&&!enforceUnit(req,bu)){cleanup();return res.status(403).json({error:'You cannot reconcile another business unit period.'})}
    if(isPeriodClosed(bu,key+'-01')){cleanup();return res.status(409).json({error:'This period is already closed. Reopen it before changing reconciliation status.'})}
    ensurePeriodReconRows(bu,key);const area=text(req.body.area_key),status=text(req.body.status),note=text(req.body.notes);if(!['Open','Reconciled','Exception Approved','Not Applicable'].includes(status)){cleanup();return res.status(400).json({error:'Invalid reconciliation status.'})}const row=db.prepare('SELECT * FROM accounting_period_reconciliations WHERE period_key=? AND area_key=? AND COALESCE(business_unit_id,0)=COALESCE(?,0)').get(key,area,bu);if(!row){cleanup();return res.status(404).json({error:'Reconciliation checklist item not found.'})}if(['Exception Approved','Not Applicable'].includes(status)&&!note){cleanup();return res.status(400).json({error:'A reason is required for Exception Approved or Not Applicable status.'})}
    const evidence=req.file?'/uploads/'+req.file.filename:row.evidence_file||'';db.prepare(`UPDATE accounting_period_reconciliations SET status=?,notes=?,evidence_file=?,completed_by=?,completed_at=CASE WHEN ?='Open' THEN NULL ELSE CURRENT_TIMESTAMP END,updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(status,note,evidence,status==='Open'?null:req.user.id,status,row.id);audit(req.user,'accounting_period_reconciliation',row.id,'status',JSON.stringify({business_unit_id:bu,period:key,area_key:area,before:row.status,after:status,notes:note,evidence:!!evidence}));res.json({ok:true,readiness:periodCloseReadiness(bu,key)});
  });
  app.post('/api/accounting/periods/close',auth,requireAccountingWrite,(req,res)=>{
    const key=text(req.body.period_key);if(!/^\d{4}-\d{2}$/.test(key))return res.status(400).json({error:'Period must be YYYY-MM.'});const companyScope=String(req.body.company_scope||'')==='1';let bu=companyScope?null:(req.body.business_unit_id?Number(req.body.business_unit_id):currentUnit(req));if(req.user.role!=='CEO / Owner'&&!bu)bu=req.user.business_unit_id;if(companyScope&&req.user.role!=='CEO / Owner')return res.status(403).json({error:'Only CEO / Owner can close the consolidated company period.'});if(bu&&!enforceUnit(req,bu))return res.status(403).json({error:'You cannot close another business unit period.'});const note=text(req.body.notes);if(!note)return res.status(400).json({error:'Period-close review note is required.'});
    const readiness=periodCloseReadiness(bu,key);if(!readiness.ready)return res.status(409).json({error:'Period close is blocked until reconciliation and accounting controls are complete.',blockers:readiness.blockers,readiness});
    const existingPeriod=db.prepare('SELECT id FROM accounting_periods WHERE period_key=? AND COALESCE(business_unit_id,0)=COALESCE(?,0) ORDER BY id LIMIT 1').get(key,bu||null);if(existingPeriod)db.prepare("UPDATE accounting_periods SET status='Closed',notes=?,closed_by=?,closed_at=CURRENT_TIMESTAMP WHERE id=?").run(note,req.user.id,existingPeriod.id);else db.prepare("INSERT INTO accounting_periods(business_unit_id,period_key,status,notes,closed_by,closed_at) VALUES(?,?,'Closed',?,?,CURRENT_TIMESTAMP)").run(bu||null,key,note,req.user.id);audit(req.user,'accounting_period',0,'close',JSON.stringify({business_unit_id:bu||null,period:key,readiness_checks:readiness.checks,notes:note}));res.json({ok:true,readiness});
  });
  app.post('/api/accounting/periods/reopen',auth,(req,res)=>{if(req.user.role!=='CEO / Owner')return res.status(403).json({error:'Only CEO / Owner can reopen a closed accounting period.'});const key=text(req.body.period_key),bu=req.body.business_unit_id?Number(req.body.business_unit_id):null,reason=text(req.body.reason);if(!/^\d{4}-\d{2}$/.test(key))return res.status(400).json({error:'Period must be YYYY-MM.'});if(!reason)return res.status(400).json({error:'A reason is required to reopen a closed accounting period.'});const r=db.prepare("UPDATE accounting_periods SET status='Open',reopened_by=?,reopened_at=CURRENT_TIMESTAMP,notes=COALESCE(notes,'')||? WHERE period_key=? AND COALESCE(business_unit_id,0)=COALESCE(?,0)").run(req.user.id,' | REOPEN: '+reason,key,bu);if(!r.changes)return res.status(404).json({error:'Closed accounting period not found.'});db.prepare("UPDATE accounting_period_reconciliations SET status='Open',completed_by=NULL,completed_at=NULL,notes=CASE WHEN notes='' THEN 'Reopened period requires fresh reconciliation sign-off.' ELSE notes||' | Reopened period requires fresh reconciliation sign-off.' END,updated_at=CURRENT_TIMESTAMP WHERE period_key=? AND COALESCE(business_unit_id,0)=COALESCE(?,0)").run(key,bu);audit(req.user,'accounting_period',0,'reopen',JSON.stringify({business_unit_id:bu,period:key,reason}));res.json({ok:true})});
  ...
  app.post('/api/accounting/inter-unit-transfers',auth,requireAccountingWrite,(req,res)=>{
    ...
    const transferNo=numbering.next('accounting.inter_unit_transfer',{businessUnitId:from});
  ...
  app.get('/api/accounting/integrity',auth,allow('finance','dashboard'),(req,res)=>{const f=unitFilter(req,'j'),unbalanced=db.prepare(`SELECT COUNT(*) c FROM (SELECT j.id,ROUND(SUM(l.debit_krw-l.credit_krw),2) diff FROM accounting_journal_entries j JOIN accounting_journal_lines l ON l.journal_entry_id=j.id WHERE j.status IN ('Posted','Reversed')${f.sql} GROUP BY j.id HAVING ABS(diff)>0.01)`).get(...f.args).c,financeUnposted=req.selected_business_unit_id?db.prepare("SELECT COUNT(*) c FROM finance_entries WHERE status!='Voided' AND accounting_status NOT IN ('Posted','Skipped','Reversed','Pending Review','Correction Required') AND business_unit_id=?").get(req.selected_business_unit_id).c:req.user.role==='CEO / Owner'?db.prepare("SELECT COUNT(*) c FROM finance_entries WHERE status!='Voided' AND accounting_status NOT IN ('Posted','Skipped','Reversed','Pending Review','Correction Required')").get().c:db.prepare("SELECT COUNT(*) c FROM finance_entries WHERE status!='Voided' AND accounting_status NOT IN ('Posted','Skipped','Reversed','Pending Review','Correction Required') AND business_unit_id=?").get(req.user.business_unit_id).c,suspense=db.prepare(`SELECT ROUND(COALESCE(SUM(ABS(l.debit_krw-l.credit_krw)),0),2) v FROM accounting_journal_lines l JOIN accounting_journal_entries j ON j.id=l.journal_entry_id JOIN accounting_accounts a ON a.id=l.account_id WHERE j.status IN ('Posted','Reversed') AND a.subtype='Suspense'${f.sql}`).get(...f.args).v,exceptions=req.selected_business_unit_id?db.prepare("SELECT COUNT(*) c FROM accounting_exceptions WHERE status='Open' AND business_unit_id=?").get(req.selected_business_unit_id).c:req.user.role==='CEO / Owner'?db.prepare("SELECT COUNT(*) c FROM accounting_exceptions WHERE status='Open'").get().c:db.prepare("SELECT COUNT(*) c FROM accounting_exceptions WHERE status='Open' AND business_unit_id=?").get(req.user.business_unit_id).c;const pendingReview=req.selected_business_unit_id?db.prepare("SELECT COUNT(*) c FROM accounting_journal_entries WHERE status IN ('Pending Review','Correction Required') AND business_unit_id=?").get(req.selected_business_unit_id).c:req.user.role==='CEO / Owner'?db.prepare("SELECT COUNT(*) c FROM accounting_journal_entries WHERE status IN ('Pending Review','Correction Required')").get().c:db.prepare("SELECT COUNT(*) c FROM accounting_journal_entries WHERE status IN ('Pending Review','Correction Required') AND business_unit_id=?").get(req.user.business_unit_id).c;res.json({ok:!Number(unbalanced)&&!Number(financeUnposted)&&!Number(exceptions)&&!Number(pendingReview),unbalanced_journals:Number(unbalanced||0),finance_not_posted:Number(financeUnposted||0),pending_posting_review:Number(pendingReview||0),open_exceptions:Number(exceptions||0),suspense_activity_krw:Number(suspense||0),sync_queue:Number(db.prepare('SELECT COUNT(*) c FROM accounting_sync_queue').get().c||0)})});

  return {version:VERSION,processQueue,scheduleQueue,syncFinanceEntry,postJournal,reverseJournal,accountId,isPeriodClosed};
}
