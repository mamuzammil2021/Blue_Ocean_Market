...
function install({app,db,auth,allow,currentUnit,enforceUnit,isFinanceReviewer,audit,notify,uploads,hasAccess,upload:sharedUpload}){
  ...
  function periodCloseReadiness(businessUnitId,key){
    ...
    const unverified=bu?db.prepare("SELECT COUNT(*) c FROM finance_entries WHERE business_unit_id=? AND status!='Voided' AND substr(COALESCE(transaction_date,created_at),1,7)=? AND COALESCE(verification_status,'') NOT IN ('Verified','Verified / Correct')").get(bu,key).c:db.prepare("SELECT COUNT(*) c FROM finance_entries WHERE status!='Voided' AND substr(COALESCE(transaction_date,created_at),1,7)=? AND COALESCE(verification_status,'') NOT IN ('Verified','Verified / Correct')").get(key).c;
