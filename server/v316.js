...
function install(ctx){
  ...
  function effectiveAccess(userId,bu=null){
    ...
    const storedUnits=assignedUnits(u.id),units=u.role==='CEO / Owner'?db.prepare("SELECT id,name,status,CASE WHEN id=? THEN 1 ELSE 0 END is_primary FROM business_units WHERE status!='Archived' ORDER BY CASE WHEN id=? THEN 0 ELSE 1 END,name").all(Number(u.business_unit_id||0),Number(u.business_unit_id||0)):storedUnits,selected=bu?Number(bu):(u.business_unit_id||units.find(x=>x.is_primary)?.id||units[0]?.id||null),legacy=templateForRole(u.role);
    let value;
    if(u.role==='CEO / Owner'){
      let allKeys=[...MODULES.flatMap(m=>ACTIONS.map(a=>permissionKey(m,a))),...SENSITIVE,...DELEGATED];try{if(db.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='access_permission_catalog_v342'").get())allKeys=db.prepare("SELECT permission_key FROM access_permission_catalog_v342 WHERE active=1 ORDER BY permission_key").all().map(x=>x.permission_key)}catch(_){}value={user:u,business_unit_id:selected,permissions:new Proxy({}, {get:()=>true}),permissions_plain:Object.fromEntries(allKeys.map(k=>[k,true])),limits:{},assigned_units:units,template:'CEO / Owner',profiles:[{role_key:'ceo_owner',role_name:'CEO / Owner',business_unit_id:null,is_primary:1}],groups:[],permission_sources:{},full_system_access:true};
    }else{
  ...
  function userSummary(u){const a=u.role==='CEO / Owner'?db.prepare("SELECT id,name,status,CASE WHEN id=? THEN 1 ELSE 0 END is_primary FROM business_units WHERE status!='Archived' ORDER BY CASE WHEN id=? THEN 0 ELSE 1 END,name").all(Number(u.business_unit_id||0),Number(u.business_unit_id||0)):assignedUnits(u.id);return {...u,assigned_units:a,primary_business_unit_id:u.role==='CEO / Owner'?null:(u.business_unit_id||a.find(x=>x.is_primary)?.id||null),full_system_access:u.role==='CEO / Owner'}}
  ...
  app.get('/api/access/catalog',auth,(req,res)=>{let permissions=[];try{if(db.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='access_permission_catalog_v342'").get())permissions=db.prepare("SELECT permission_key,permission_type,module_key,action_key,label_en,label_ko,description,sensitive,introduced_version FROM access_permission_catalog_v342 WHERE active=1 ORDER BY permission_type,module_key,action_key,permission_key").all()}catch(_){}res.json({modules:MODULES,actions:ACTIONS,sensitive:SENSITIVE,delegated:DELEGATED,limits:LIMITS,finance_modules:[...FINANCE_MODULES],permissions})});
  ...
  app.put('/api/access/users/:id/assignments',auth,(req,res)=>{
    const target=liveUser(req.params.id);if(!target)return res.status(404).json({error:'User not found'});if(target.role==='CEO / Owner')return res.status(409).json({error:'CEO / Owner access is system-managed as Full System Access and cannot be reduced with Business Unit assignments.'});
  ...
  app.put('/api/access/users/:id/overrides',auth,(req,res)=>{
    const target=liveUser(req.params.id);if(!target)return res.status(404).json({error:'User not found'});if(target.role==='CEO / Owner')return res.status(409).json({error:'CEO / Owner permissions are system-managed as Full System Access and cannot be denied or overridden.'});const scope=req.selected_business_unit_id||req.user.business_unit_id;
  ...
  app.put('/api/access/users/:id/limits',auth,(req,res)=>{
    const target=liveUser(req.params.id);if(!target)return res.status(404).json({error:'User not found'});if(target.role==='CEO / Owner')return res.status(409).json({error:'CEO / Owner access limits are system-managed and are not configured through ordinary user access limits.'});const scope=req.selected_business_unit_id||req.user.business_unit_id;
    if(req.user.role!=='CEO / Owner'&&!requireAccessDelegation(req,scope))return res.status(403).json({error:'Manage access permissions is not delegated to your account.'});
  ...
  app.post('/api/access/users/:id/apply-template',auth,(req,res)=>{
    const target=liveUser(req.params.id),role=String(req.body.role||'');if(!target)return res.status(404).json({error:'User not found'});if(target.role==='CEO / Owner')return res.status(409).json({error:'CEO / Owner uses system-managed Full System Access; Access Profiles cannot replace it.'});const units=assignedUnits(target.id).map(x=>x.id),scope=req.selected_business_unit_id||target.business_unit_id;
    if(req.user.role!=='CEO / Owner'&&!requireAccessDelegation(req,scope))return res.status(403).json({error:'Manage access permissions is not delegated to your account.'});
  ...
  app.post('/api/access/profiles',auth,(req,res)=>{
    ...
    const description=String(req.body.description||'').trim().slice(0,800);const permissions=req.body.permissions&&typeof req.body.permissions==='object'?req.body.permissions:{};const limits=req.body.limits&&typeof req.body.limits==='object'?req.body.limits:{};
    ...
    db.prepare('INSERT INTO access_role_templates(role_key,role_name,description,permissions_json,limits_json,active) VALUES(?,?,?,?,?,1)').run(key,name,description,JSON.stringify(permissions),JSON.stringify(limits));invalidateAccess();audit(req.user,'access_profile',key,'create-v341',JSON.stringify({role_name:name,description}));res.json({ok:true,role_key:key,role_name:name});
  });
  app.put('/api/access/profiles/:role_key',auth,(req,res)=>{
    ...
    const name=String(req.body.role_name||current.role_name).trim();if(!name)return res.status(400).json({error:'Profile name is required.'});const description=String(req.body.description??current.description??'').trim().slice(0,800);const permissions=req.body.permissions&&typeof req.body.permissions==='object'?req.body.permissions:current.permissions;const limits=req.body.limits&&typeof req.body.limits==='object'?req.body.limits:current.limits;
    ...
    const before={role_name:current.role_name,description:current.description||'',permissions:current.permissions,limits:current.limits},affected=Number(db.prepare('SELECT COUNT(DISTINCT user_id) n FROM user_access_profiles WHERE role_key=?').get(current.role_key)?.n||0);db.prepare('UPDATE access_role_templates SET role_name=?,description=?,permissions_json=?,limits_json=?,updated_at=CURRENT_TIMESTAMP WHERE role_key=?').run(name,description,JSON.stringify(permissions),JSON.stringify(limits),current.role_key);if(name!==current.role_name)db.prepare('UPDATE users SET role=? WHERE role=?').run(name,current.role_name);invalidateAccess();audit(req.user,'access_profile',current.role_key,'update-v340',JSON.stringify({before,after:{role_name:name,description,permissions,limits},affected_users:affected}));res.json({ok:true,role_key:current.role_key,role_name:name,affected_users:affected});
  });
  ...
  app.put('/api/access/users/:id/profiles',auth,(req,res)=>{
    const target=liveUser(req.params.id);if(!target)return res.status(404).json({error:'User not found'});if(target.role==='CEO / Owner')return res.status(409).json({error:'CEO / Owner uses system-managed Full System Access; Access Profile assignment is not required.'});const admin=accessAdminAllowed(req,target);if(!admin.ok)return res.status(403).json({error:admin.error});
  ...
  app.put('/api/access/users/:id/groups',auth,(req,res)=>{
    const target=liveUser(req.params.id);if(!target)return res.status(404).json({error:'User not found'});if(target.role==='CEO / Owner')return res.status(409).json({error:'CEO / Owner uses system-managed Full System Access; Permission Group assignment is not required.'});const admin=accessAdminAllowed(req,target);if(!admin.ok)return res.status(403).json({error:admin.error});const rows=Array.isArray(req.body.assignments)?req.body.assignments:[],allowedUnits=new Set(assignedUnits(target.id).map(x=>Number(x.id))),clean=[];
  ...
  app.post('/api/access/profiles/:role_key/duplicate',auth,(req,res)=>{
    if(!canManageProfileLibrary(req))return res.status(403).json({error:'Only CEO / Owner or an authorized System Administrator can manage the shared Access Profile library.'});const src=templateForKey(req.params.role_key);if(!src.role_name)return res.status(404).json({error:'Access Profile not found'});const name=String(req.body.role_name||`${src.role_name} Copy`).trim();if(!name)return res.status(400).json({error:'Profile name is required.'});let key=roleKey(name),n=2;while(db.prepare('SELECT 1 FROM access_role_templates WHERE role_key=? OR role_name=?').get(key,name))key=roleKey(name+' '+n++);db.prepare('INSERT INTO access_role_templates(role_key,role_name,description,permissions_json,limits_json,active) VALUES(?,?,?,?,?,1)').run(key,name,src.description||'',JSON.stringify(src.permissions||{}),JSON.stringify(src.limits||{}));audit(req.user,'access_profile',key,'duplicate',JSON.stringify({source:src.role_key,name}));res.json({ok:true,role_key:key,role_name:name});
  });

  return {MODULES,ACTIONS,SENSITIVE,DELEGATED,LIMITS,liveUser,assignedUnits,isAssigned,effectiveAccess,can,canModule,canAction,canAnyModule,getLimit,canManageTarget,validateOverrides,validateLimits,replaceAssignments,userSummary,ensureInitialProfile,templateForRole,templateForKey,assignedProfiles,assignedGroups,approvalAuthority,history,scopeTarget,actorUnits,normalizeModule,accessSnapshot,validUnitIds,invalidateAccess,defaultTemplates,defaultPermissionGroups,roleKey,permissionKey,cache_ttl_ms:ACCESS_CACHE_TTL_MS};
}
module.exports={install,ACTIONS,MODULES,SENSITIVE,DELEGATED,LIMITS,permissionKey,defaultTemplates,defaultPermissionGroups,roleKey};
