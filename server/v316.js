...
function install(ctx){
  ...
  function effectiveAccess(userId,bu=null){
    ...
    const u=liveUser(id);if(!u||!u.active)return {user:null,business_unit_id:null,permissions:{},permissions_plain:{},limits:{},assigned_units:[]};
    ...
    let value;
    if(u.role==='CEO / Owner')value={user:u,business_unit_id:selected,permissions:new Proxy({}, {get:()=>true}),permissions_plain:Object.fromEntries([...MODULES.flatMap(m=>ACTIONS.map(a=>permissionKey(m,a))),...SENSITIVE,...DELEGATED].map(k=>[k,true])),limits:{},assigned_units:units,template:t.role_name};
    else{const permissions={...t.permissions};for(const r of overrides(u.id,selected)){if(r.effect==='Allow')permissions[r.permission_key]=true;else if(r.effect==='Deny')permissions[r.permission_key]=false;}const limits={...t.limits};for(const r of limitRows(u.id,selected))limits[r.limit_key]=r.limit_value;value={user:u,business_unit_id:selected,permissions,permissions_plain:permissions,limits,assigned_units:units,template:t.role_name}}
