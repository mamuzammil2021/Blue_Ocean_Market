...
function install({app,db,auth,allow,audit,upload,financeSync,currentUnit,enforceUnit,excavatorGuard}){
  ...
  function payeeAccount(id,{bu,entityType='',entityId=0,method='',required=false}={}){
    ...
    const wanted=methodType(method);if(method&&wanted!=='Other'&&methodType(r.method_type)!==wanted&&!(wanted==='Bank'&&methodType(r.method_type)==='Bank')){const e=new Error(`Selected receiver account is not compatible with ${method}.`);e.status=400;throw e}
    return r;
  }
  ...
  app.post('/api/v330/excavator/buyers/:id/resale-credit/refund',auth,allow('finance'),upload.single('receipt'),(req,res)=>{const g=buyerGuard(req,res);if(!g)return;try{const available=buyerCredits(g.buyer.id).reduce((n,p)=>n+p.available_credit_pkr,0),amount=round2(req.body.amount_pkr),fx=num(req.body.fx_rate_to_krw),date=text(req.body.refund_date),reference=text(req.body.reference),reason=text(req.body.reason),account=pakistanCompanyAccount(req.body.payment_account_id,g.bu),receiver=payeeAccount(req.body.receiver_account_id,{bu:g.bu,entityType:'excavator_buyer',entityId:g.buyer.id,method:req.body.method||'Bank',required:true});if(amount<=0||amount>available+.005)throw new Error(`Refund cannot exceed available resale credit PKR ${round2(available).toLocaleString()}.`);if(fx<=0)throw new Error('FX Rate to KRW must be greater than zero.');if(!date||!reference||!reason)throw new Error('Refund date, reference and reason are required.');if(!req.file)throw new Error('Refund evidence / receipt is required.');const krw=round2(amount*fx);const out=db.transaction(()=>{const rr=db.prepare(`INSERT INTO excavator_resale_credit_refunds(buyer_id,business_unit_id,amount_pkr,fx_rate_to_krw,krw_amount,refund_date,method,reference,payment_account_id,receiver_account_id,receipt_file,reason,status,created_by) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,'Completed',?)`).run(g.buyer.id,g.bu,amount,fx,krw,date,text(req.body.method||'Bank'),reference,account.id,receiver.id,req.file.filename,reason,req.user.id),rid=Number(rr.lastInsertRowid);let left=amount;const ins=db.prepare('INSERT INTO excavator_resale_credit_refund_sources(refund_id,payment_id,amount_pkr) VALUES(?,?,?)');for(const p of buyerCredits(g.buyer.id)){if(left<=.005)break;const take=round2(Math.min(left,p.available_credit_pkr));if(take>.005){ins.run(rid,p.id,take);left=round2(left-take)}}if(left>.005)throw new Error('Resale credit changed while refund was being recorded. Refresh and try again.');const financeId=financeSync({businessUnitId:g.bu,type:'Expense',category:'Pakistan Resale Profit Refund',amount:krw,description:`Refund Pakistan resale unallocated credit · ${g.buyer.name}`,paymentMethod:text(req.body.method||'Bank'),reference,paymentAccountId:account.id,createdBy:req.user.id,sourceType:'Pakistan Resale Profit Refund',sourceId:rid,receiptFile:req.file.filename,originalAmount:amount,originalCurrency:'PKR',fxRate:fx,transactionDate:date,sourceLabel:'Excavator → Pakistan Resale Credit Refund',sourceRecordId:g.buyer.id,receiverAccountId:receiver.id,receiverEntityType:'excavator_buyer',receiverEntityId:g.buyer.id,receiverName:g.buyer.name});return {refund_id:rid,finance_id:financeId}})();audit(req.user,'excavator_resale_credit_refund',out.refund_id,'create-v330',JSON.stringify({buyer_id:g.buyer.id,amount_pkr:amount,finance_id:out.finance_id}));res.json({ok:true,...out,available_credit_after_pkr:round2(available-amount)})}catch(e){res.status(e.status||400).json({error:e.message,code:e.code})}});
