...
function install({app,db,auth,allow,currentUnit,enforceUnit,audit,uploads}){
  ...
  async function renderPdf(id,bu,lang,userId=null){const d=productionData(id,bu);if(!d)throw new Error('Production batch not found.');if(d.batch.status!=='Completed')throw new Error('Production Batch PDF is available only after repacking is completed.');const chrome=chromePath();if(!chrome)throw new Error('PDF generation requires Google Chrome or Chromium on the server. Set CHROME_PATH if Chrome is installed in a custom location.');const tmp=path.join(os.tmpdir(),`bom-production-${id}-${lang}-${Date.now()}.html`),out=pdfPath(id,lang);fs.writeFileSync(tmp,htmlReport(d,lang),'utf8');try{await renderChromePdf(chrome,['--headless','--no-sandbox','--disable-gpu',`--print-to-pdf=${out}`,`file://${tmp}`],{timeout:30000})}finally{try{fs.unlinkSync(tmp)}catch(_){}}if(!fs.existsSync(out)||fs.statSync(out).size<500)throw new Error('PDF generation failed. Chrome/Chromium could not render the production report.');upsertDoc(d.batch,lang,out,userId);return {out,batch:d.batch}}

  app.post('/api/pink-salt/production/:id/pdf-v308/generate',auth,allow('inventory','purchases','dashboard','finance'),async (req,res)=>{const bu=guard(req,res);if(!bu)return;try{const en=await renderPdf(req.params.id,bu,'en',req.user.id),ko=await renderPdf(req.params.id,bu,'ko',req.user.id);audit(req.user,'pink_salt_production',Number(req.params.id),'generate-production-pdf',JSON.stringify({en:path.basename(en.out),ko:path.basename(ko.out)}));res.json({ok:true,en:path.basename(en.out),ko:path.basename(ko.out)})}catch(e){res.status(409).json({error:e.message})}});
  app.get('/api/pink-salt/production/:id/pdf-v308',auth,allow('inventory','purchases','dashboard','finance'),async (req,res)=>{const bu=guard(req,res);if(!bu)return;const lang=String(req.query.lang||req.headers['x-language']||'en').toLowerCase().startsWith('ko')?'ko':'en';try{let out=pdfPath(req.params.id,lang),batch=db.prepare('SELECT * FROM pink_salt_production_batches WHERE id=? AND business_unit_id=?').get(req.params.id,bu);if(!batch)return res.status(404).json({error:'Production batch not found.'});if(batch.status!=='Completed')return res.status(409).json({error:'Production Batch PDF is available only after repacking is completed.'});if(!fs.existsSync(out)){const r=await renderPdf(req.params.id,bu,lang,req.user.id);out=r.out;batch=r.batch}else upsertDoc(batch,lang,out,req.user.id);res.download(out,`${String(batch.batch_no||'production_batch').replace(/[^a-z0-9_-]+/gi,'_')}_${lang}.pdf`)}catch(e){res.status(409).json({error:e.message})}});
