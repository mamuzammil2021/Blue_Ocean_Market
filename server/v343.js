...
function install({app,db,auth,allow,audit,currentUnit,enforceUnit,access,accounting}){
  ...
  function accountEvents(accountId){
    ...
    events.sort((a,b)=>String(a.date||'').localeCompare(String(b.date||''))||String(a.entered_at||'').localeCompare(String(b.entered_at||''))||Number(a.sort)-Number(b.sort)||String(a.id).localeCompare(String(b.id)));
    return events;
  }
  ...
  // V30.55: read-only account overview; account-level movements are NOT mislabelled
  // as posted GL or bank-confirmed available funds. One scoped response for cards/table.
  function accountBalanceSummary(req,a){
    const events=accountEvents(a.id),todayDate=today(),monthStart=todayDate.slice(0,7)+'-01';
    const current=round2(num(a.opening_balance_krw)+events.filter(e=>e.date<=todayDate).reduce((n,e)=>n+num(e.inflow)-num(e.outflow),0));
    const month=events.filter(e=>e.date>=monthStart&&e.date<=todayDate);
    const pending=events.filter(e=>e.id[0]==='F'&&e.date<=todayDate&&e.accounting_status!=='Posted'&&e.accounting_status!=='Reversed');
    const mappings=db.prepare('SELECT COUNT(*) c FROM accounting_payment_accounts WHERE ledger_account_id=?').get(a.ledger_account_id).c;
    const posted=mappings===1?db.prepare(`SELECT ROUND(COALESCE(SUM(l.debit_krw-l.credit_krw),0),2) balance FROM accounting_journal_lines l JOIN accounting_journal_entries j ON j.id=l.journal_entry_id WHERE l.account_id=? AND j.status IN ('Posted','Reversed') AND date(j.transaction_date)<=date(?)`).get(a.ledger_account_id,todayDate):null;
    const recon=tableExists('accounting_bank_statement_lines')?db.prepare(`SELECT COUNT(*) total, SUM(CASE WHEN status='Reconciled' THEN 1 ELSE 0 END) matched, MAX(CASE WHEN status='Reconciled' THEN statement_date END) last_reconciled_date FROM accounting_bank_statement_lines WHERE payment_account_id=?`).get(a.id):null;
    return {id:Number(a.id),recorded_balance_krw:current,period_from:monthStart,period_to:todayDate,money_in_krw:round2(month.reduce((n,e)=>n+num(e.inflow),0)),money_out_krw:round2(month.reduce((n,e)=>n+num(e.outflow),0)),pending_accounting_count:pending.length,pending_accounting_krw:round2(pending.reduce((n,e)=>n+num(e.inflow)-num(e.outflow),0)),posted_gl_balance_krw:posted?round2(posted.balance):null,shared_ledger:mappings!==1,reconciliation:{total:Number(recon?.total||0),matched:Number(recon?.matched||0),unmatched:Number(recon?.total||0)-Number(recon?.matched||0),last_reconciled_date:recon?.last_reconciled_date||null},balance_basis:'Recorded account movements in KRW, including unposted Finance movements; not bank-confirmed available funds.'};
  }
  app.get('/api/v355/accounting/payment-accounts/overview',auth,allow('finance','accounting'),(req,res)=>{
    const accounts=db.prepare(`SELECT p.*,b.name business_unit,a.code ledger_code,a.name ledger_name FROM accounting_payment_accounts p LEFT JOIN business_units b ON b.id=p.business_unit_id LEFT JOIN accounting_accounts a ON a.id=p.ledger_account_id ORDER BY p.active DESC,p.name`).all().filter(a=>canSeeAccount(req,a));
    res.set('Cache-Control','no-store');res.json({as_of:today(),accounts:accounts.map(a=>({...publicAccount(a),...accountBalanceSummary(req,a)}))});
  });
  ...
  app.get('/api/v343/accounting/payment-accounts/:id/detail',auth,allow('finance','accounting'),(req,res)=>{const s=accountStatement(req,res,req.params.id);if(!s)return;const summary=accountBalanceSummary(req,accountRow(req.params.id));const peers=db.prepare(`SELECT p.id,p.name,p.currency,p.business_unit_id,b.name business_unit,COALESCE(p.financial_account_type,p.payment_type) payment_type FROM accounting_payment_accounts p LEFT JOIN business_units b ON b.id=p.business_unit_id WHERE p.active=1 AND COALESCE(p.status,'Active')='Active' AND p.id<>? ORDER BY b.name,p.name`).all(Number(req.params.id)).filter(x=>canSeeAccount(req,x));res.json({...s,summary,transfer_accounts:peers,can_transfer:canTransfer(req,accountRow(req.params.id))})});
