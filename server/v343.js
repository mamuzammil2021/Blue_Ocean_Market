...
function install({app,db,auth,allow,audit,currentUnit,enforceUnit,access,accounting}){
  ...
  function accountEvents(accountId){
    ...
    if(tableExists('finance_entries')){
      const rows=db.prepare(`SELECT id,COALESCE(transaction_date,substr(created_at,1,10)) tx_date,type,category,description,reference,krw_amount,amount,original_amount,original_currency,fx_rate,cash_effect,verification_status,accounting_status,source_type,source_id,status
    ...
    if(tableExists('payroll_payments')){
      ...
      for(const r of rows){const amt=round2(r.amount_krw);events.push({date:r.payment_date,sort:30,id:`PP${r.id}`,type:'Payroll Payment',reference:r.reference||'',description:`Payroll ${r.payroll_no||r.payroll_run_id||''}`,source:'Payroll Payment',source_id:r.id,inflow:0,outflow:amt,verification_status:'',accounting_status:'Posted',original_amount:amt,original_currency:'KRW',fx_rate:1});}
    }
    ...
    if(tableExists('excavator_resale_bank_transfers')){
      ...
      for(const r of rows){const isFrom=Number(r.from_payment_account_id)===id,sourceKrw=round2(r.source_krw_equivalent),destKrw=round2(r.destination_amount_krw),feeKrw=round2(r.bank_fee_krw),amt=isFrom?sourceKrw+feeKrw:destKrw;events.push({date:r.transfer_date,sort:22,id:`PK${r.id}`,type:'Pakistan → Korea Bank Transfer',reference:r.reference||'',description:isFrom?'Transfer out to Korea'+(feeKrw?` · Fee ₩ ${feeKrw.toLocaleString()}`:''):'Transfer received from Pakistan',source:'Pakistan Resale Bank Transfer',source_id:r.id,inflow:isFrom?0:amt,outflow:isFrom?amt:0,verification_status:'',accounting_status:'Posted',original_amount:isFrom?num(r.source_amount_pkr):num(r.destination_amount_krw),original_currency:isFrom?'PKR':'KRW',fx_rate:num(r.fx_rate_to_krw||1)});}
    }
    events.sort((a,b)=>String(a.date).localeCompare(String(b.date))||Number(a.sort)-Number(b.sort)||String(a.id).localeCompare(String(b.id)));
    return events;
  }
