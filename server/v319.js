...
function install({app,db,auth,currentUnit,enforceUnit,audit,notify,access,uploads,secret}){
  ...
  // Legacy synchronous helper is retained only for the bounded startup integrity backfill below.
  const hashFile=filePath=>{try{return crypto.createHash('sha256').update(fs.readFileSync(filePath)).digest('hex')}catch(_){return ''}};
  // Interactive uploads must not reread a potentially large file synchronously on the Node event loop.
  const hashFileAsync=filePath=>new Promise(resolve=>{try{const h=crypto.createHash('sha256'),stream=fs.createReadStream(filePath);stream.on('data',chunk=>h.update(chunk));stream.on('end',()=>resolve(h.digest('hex')));stream.on('error',()=>resolve(''))}catch(_){resolve('')}});
  ...
  // -------------------------------------------------------------------------
  // V30.22: application secrets use a dedicated encryption key when configured.
  // Legacy JWT-derived encryption remains readable so existing SMTP/AI secrets are
  // not lost during upgrade; newly saved secrets use APP_ENCRYPTION_KEY.
  const legacySecretKey=crypto.createHash('sha256').update(String(secret||process.env.JWT_SECRET||'BlueOceanLocalOnly')).digest();
  const encryptionMaterial=String(process.env.APP_ENCRYPTION_KEY||'').trim();
  const secretKey=encryptionMaterial?crypto.createHash('sha256').update(encryptionMaterial).digest():legacySecretKey;
  ...
  function decryptWithKey(row,key){try{const dec=crypto.createDecipheriv('aes-256-gcm',key,Buffer.from(row.iv,'base64'));dec.setAuthTag(Buffer.from(row.auth_tag,'base64'));return Buffer.concat([dec.update(Buffer.from(row.ciphertext,'base64')),dec.final()]).toString('utf8')}catch(_){return ''}}
  function decryptSecret(row){if(!row)return '';const primary=decryptWithKey(row,secretKey);if(primary)return primary;return secretKey.equals(legacySecretKey)?'':decryptWithKey(row,legacySecretKey)}
  ...
  app.get('/api/system-settings-v319/overview',auth,(req,res)=>{
    ...
    if(!smtp.smtp_enabled)warnings.push({code:'SMTP_DISABLED',level:'warning',message:'Outgoing email is disabled. Password reset links will not be emailed in production.'});if(!hasSecret('company',0,'smtp_password')&&smtp.smtp_username)warnings.push({code:'SMTP_PASSWORD_MISSING',level:'warning',message:'SMTP username is configured but no secure SMTP password is stored.'});if(String(process.env.NODE_ENV||'development').toLowerCase()==='production'&&!String(process.env.APP_ENCRYPTION_KEY||'').trim())warnings.push({code:'APP_ENCRYPTION_KEY_MISSING',level:'critical',message:'Set APP_ENCRYPTION_KEY in production so SMTP/AI secrets are encrypted separately from JWT signing.'});if(String(process.env.NODE_ENV||'development').toLowerCase()==='production'&&!String(process.env.APP_BASE_URL||'').trim())warnings.push({code:'APP_BASE_URL_MISSING',level:'critical',message:'Set APP_BASE_URL in production for trusted password-reset links.'});if(pendingMigrations)warnings.push({code:'PENDING_MIGRATIONS',level:'warning',message:`${pendingMigrations} migration job(s) are not finalized.`});if(!dbOk)warnings.push({code:'DATABASE_ERROR',level:'critical',message:dbError});
  ...
  // Secure forgot-password override. Public authentication is isolated from
  // business contact validation. Reset tokens are stored only as SHA-256 hashes.
  const resetRate=new Map();
  app.post('/api/auth/forgot',async(req,res)=>{
    const email=text(req.body?.email).toLowerCase(),ip=text(req.ip||req.socket?.remoteAddress||'unknown'),key=crypto.createHash('sha256').update(ip+'|'+email).digest('hex'),nowMs=Date.now(),windowMs=15*60*1000,limit=5,old=resetRate.get(key);if(!old||nowMs-old.start>windowMs)resetRate.set(key,{start:nowMs,count:1});else{old.count++;if(old.count>limit)return res.status(429).json({message:'If the account exists, password-reset instructions will be sent shortly.'})}
    ...
    const token=crypto.randomBytes(32).toString('hex'),tokenHash=crypto.createHash('sha256').update(token).digest('hex'),prod=String(process.env.NODE_ENV||'development').toLowerCase()==='production',configuredBase=text(process.env.APP_BASE_URL).replace(/\/$/,''),base=configuredBase||(!prod?`${req.protocol}://${req.get('host')}`:''),resetUrl=base?`${base}/?reset=${token}`:'',smtp=smtpConfig();
    db.prepare("UPDATE password_resets SET used=1 WHERE user_id=? AND used=0").run(u.id);db.prepare("INSERT INTO password_resets(user_id,token,token_hash,expires_at) VALUES(?,?,?,datetime('now','+30 minutes'))").run(u.id,'hash:'+tokenHash,tokenHash);
    let emailStatus='Not Sent';
    if(smtp.smtp_enabled&&resetUrl){try{await smtpSend(smtp,{to:u.email,subject:'Blue Ocean Market password reset',textBody:`A password reset was requested for your Blue Ocean Market account.\n\nOpen this link within 30 minutes:\n${resetUrl}\n\nIf you did not request this, ignore this email.`});emailStatus='Sent';logEmail({purpose:'Password Reset',recipient:u.email,subject:'Blue Ocean Market password reset',status:'Sent'})}catch(e){emailStatus='Failed';logEmail({purpose:'Password Reset',recipient:u.email,subject:'Blue Ocean Market password reset',status:'Failed',error:e.message})}}
    else if(smtp.smtp_enabled&&!resetUrl){emailStatus='Configuration Error';logEmail({purpose:'Password Reset',recipient:u.email,subject:'Blue Ocean Market password reset',status:'Failed',error:'APP_BASE_URL is required in production.'})}
    audit(null,'password_reset',u.id,'requested',JSON.stringify({email_status:emailStatus,token_storage:'sha256',trusted_base_url:!!configuredBase||!prod}));if(String(process.env.LOCAL_TEST_MODE||'').toLowerCase()==='true'&&!prod&&resetUrl)response.reset_url=resetUrl;res.json(response);
  });

  // Secure password-reset completion. Password policy is centralized in System
  // Settings and successful resets revoke every previously issued user session.
  app.post('/api/auth/reset',(req,res)=>{
    try{
      const rawToken=text(req.body?.token),tokenHash=crypto.createHash('sha256').update(rawToken).digest('hex'),r=db.prepare("SELECT pr.*,u.email,u.active FROM password_resets pr JOIN users u ON u.id=pr.user_id WHERE pr.token_hash=? AND pr.used=0 AND datetime(pr.expires_at)>datetime('now')").get(tokenHash);
      ...
      db.transaction(()=>{db.prepare('UPDATE users SET password_hash=?,auth_version=COALESCE(auth_version,1)+1 WHERE id=?').run(bcrypt.hashSync(password,12),r.user_id);db.prepare('UPDATE password_resets SET used=1 WHERE user_id=? AND used=0').run(r.user_id)})();
      audit(null,'password_reset',r.user_id,'completed',JSON.stringify({policy_min_length:min,sessions_revoked:true,token_storage:'sha256'}));
  ...
  function migrationNo(bu){return numbering.next('migration.job',{businessUnitId:Number(bu||0)})}
