...
window.toggleNotifs=async function(){if(document.getElementById('notifDialog')){closeNotifDialog();return}
 try{const d=await api('/api/v348/notifications/feed?pageSize=25&status=all'),n=d.rows||[],unreadRows=n.filter(x=>!x.read_at),readRows=n.filter(x=>x.read_at),wrap=document.createElement('div');
